...
// Use env var if set, otherwise default to empty (relative)
// const ENV_API_BASE = import.meta.env.VITE_API_BASE || "";
// Two separate API Bases as per instruction
const BACKEND_API_BASE = "https://rlcslwgm-5000.inc1.devtunnels.ms";
const AI_API_BASE = "https://rlcslwgm-5000.inc1.devtunnels.ms";
...
async function fetchJson<T>(endpoint: string, options?: RequestInit): Promise<T> {
    ...
    // Append workspace_id and user_id to URL specific parameters if they aren't already there
    if (!url.searchParams.has("workspace_id") && workspaceId) {
        url.searchParams.append("workspace_id", workspaceId);
    }
    // Only append user_id if we have it and it's not already there
    if (!url.searchParams.has("user_id") && userId) {
        url.searchParams.append("user_id", userId);
    }

    // For fetch, toString() gives the full absolute URL
    const fetchUrl = url.toString();

    // Try to get token if stored in sv_user or separate key
    let token = "";
    try {
        ...
        if (userStr) {
            ...
            // distinct from user.id
        }
    } catch { }

    const response = await fetch(fetchUrl, {
        ...options,
        // Critical: Include credentials (cookies/auth headers) for devtunnels/auth access
        credentials: "include",
        headers: {
            "Content-Type": "application/json",
            ...(token ? { "Authorization": `Bearer ${token}` } : {}),
            // Also inject as headers for body-based requests (POST/PATCH)
...
export const api = {
    ...
    getDashboardStats: (startDate?: string, endDate?: string) =>
        fetchJson<{ [key: string]: Metric }>(`/api/dashboard/stats?startDate=${startDate}&endDate=${endDate}`),

    getRevenueChart: (startDate?: string, endDate?: string) =>
        fetchJson<{ date: string; value: number }[]>(`/api/dashboard/charts/revenue?startDate=${startDate}&endDate=${endDate}`),

    getSourceChart: (startDate?: string, endDate?: string) =>
        fetchJson<{ name: string; value: number }[]>(`/api/dashboard/charts/sources?startDate=${startDate}&endDate=${endDate}`),
    ...
    getConsolidatedCampaigns: (filters?: { date_preset?: string; since?: string; until?: string }) => {
        ...
        return fetchJson<any>(`https://rlcslwgm-5000.inc1.devtunnels.ms/api/meta/consolidated-campaigns?${params.toString()}`);
    },
    ...
    createContact: (contact: Partial<Contact>) => {
        ...
        return fetchJson<Contact>("/api/contacts", { method: "POST", body: JSON.stringify(payload) });
    },
    ...
    getWorkspaceUsers: () => Promise.resolve([]),
