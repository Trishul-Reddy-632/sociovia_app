...
import { MoreVertical, Phone, Mail, DollarSign, Plus, LayoutGrid, List as ListIcon, Search, Filter, GripVertical, RefreshCcw, ExternalLink } from "lucide-react";
...
export default function Leads() {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (currentLead) {
            try {
                await api.updateLead(activeLeadId, { status: currentLead.status });
            } catch (err) {
...
function ListView({
    ...
    return (
        <>
                ...
                <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50/80 text-xs uppercase text-slate-500 sticky top-0 z-0 backdrop-blur-md">
                        <tr>
                            ...
                            <th className="px-6 py-4 font-bold tracking-wider">Source / Sync</th>
                        ...
                        {leads.map((lead) => (
                            ...
                            >
                                <td className="px-6 py-4">
                                    ...
                                    {lead.company && <div className="text-xs text-slate-500 font-medium">{lead.company} {lead.job_title ? `• ${lead.job_title}` : ''}</div>}
                                </td>
                                <td className="px-6 py-4">
                                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold capitalize 
                                        ${lead.status === 'qualified' ? 'bg-violet-100 text-violet-600' :
                                            lead.status === 'new' ? 'bg-blue-100 text-blue-600' :
                                                lead.status === 'closed' ? 'bg-emerald-100 text-emerald-600' :
                                                    lead.status === 'proposal' ? 'bg-pink-100 text-pink-600' :
                                ...
                                <td className="px-6 py-4 text-right font-bold text-slate-900">${lead.value?.toLocaleString() ?? '-'}</td>
            ...
            {/* Pagination Footer */}
            <div className="flex items-center justify-between border-t border-slate-100 p-4 bg-slate-50/50 backdrop-blur text-sm">
