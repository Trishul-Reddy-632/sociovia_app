...
import {
    ...
    ArrowRight, DollarSign, Edit, Trash2, Ban, Archive, ChevronDown
} from "lucide-react";
                        ...
                        <div className="flex-1 overflow-y-auto pt-6 px-6 pb-6 space-y-8 scrollbar-hide">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                    <DropdownMenu>
                                        <DropdownMenuTrigger className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 font-medium outline-none focus:ring-2 focus:ring-violet-500/20 flex items-center justify-between">
                                            {STAGES.find(s => s.id === deal.stage)?.label || deal.stage}
                                            <ChevronDown className="h-4 w-4 text-slate-400" />
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width]">
                                            {STAGES.map(s => (
                                                <DropdownMenuItem key={s.id} onClick={() => handleStageChange(s.id)} className="font-medium">
                                                    {s.label}
                                                </DropdownMenuItem>
                                            ))}
                                        </DropdownMenuContent>
