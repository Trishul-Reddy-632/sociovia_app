...
import { X, Phone, Mail, Calendar, MapPin, Tag, Clock, CheckCircle, AlertCircle, MessageSquare, Briefcase, ArrowRight, Edit2, Linkedin, Twitter, Globe, Facebook, ChevronDown } from "lucide-react";
            ...
            {isOpen && lead && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed inset-y-0 right-0 z-[60] w-full sm:w-[500px] bg-white/95 backdrop-blur-xl shadow-2xl border-l border-white/20 flex flex-col overflow-hidden"
                    >
                        {/* Header Image/Gradient */}
                        <div className="bg-white border-b border-slate-100 relative shrink-0 p-6 pt-12">
                            <button
                                onClick={onClose}
                                className="absolute top-4 right-4 rounded-full p-2 bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-700 transition-colors z-10"
                            >
                                <X className="h-5 w-5" />
                            </button>

                            <div className="flex items-start gap-5">
                                <div className="h-20 w-20 rounded-2xl bg-white p-1 shadow-lg shadow-slate-200/50 shrink-0 border border-slate-100">
                                    <div className="h-full w-full bg-gradient-to-br from-violet-100 to-indigo-50 rounded-xl flex items-center justify-center text-3xl font-bold text-violet-600">
                                        {lead.name.charAt(0)}
                                    </div>
                                </div>
                                <div className="flex-1 min-w-0 pt-1">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <h2 className="text-2xl font-bold text-slate-900 tracking-tight truncate">{lead.name}</h2>
                                            <div className="flex items-center gap-3 mt-2 text-slate-500 text-sm font-medium">
                                                <div className="flex items-center gap-2 bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200 hover:bg-slate-200/50 transition-colors">
                                                    <div className={`w-2 h-2 rounded-full ${lead.status === 'new' ? 'bg-blue-500' : lead.status === 'closed' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger className="bg-transparent border-none text-slate-700 text-xs font-bold uppercase focus:ring-0 cursor-pointer outline-none flex items-center gap-1">
                                                            {lead.status}
                                                            <ChevronDown className="h-3 w-3 opacity-50" />
                                                        </DropdownMenuTrigger>
                                                        <DropdownMenuContent align="start" className="w-[140px]">
                                                            <DropdownMenuItem onClick={() => handleUpdateField("status", "new")}>New</DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => handleUpdateField("status", "contacted")}>Contacted</DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => handleUpdateField("status", "qualified")}>Qualified</DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => handleUpdateField("status", "proposal")}>Proposal</DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => handleUpdateField("status", "closed")}>Closed</DropdownMenuItem>
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
                                                </div>
                                                <span className="truncate flex items-center gap-1.5 opacity-80">
                                        ...
                                        <button
                                            ...
                                            className={`rounded-xl p-2.5 transition-all border ${editMode ? 'bg-violet-50 text-violet-600 border-violet-200 shadow-sm' : 'bg-white text-slate-400 border-slate-200 hover:border-slate-300 hover:text-slate-600'}`}
