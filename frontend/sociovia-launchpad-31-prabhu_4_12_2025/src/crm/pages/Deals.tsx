...
function ListView({ deals, setSelectedDeal, page, setPage, itemsPerPage }: {
    deals: Deal[];
    setSelectedDeal: (d: Deal) => void;
    page: number;
    ...
    const paginatedDeals = deals.slice(start, start + itemsPerPage);
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between border-t border-slate-100 p-4 bg-slate-50/50 backdrop-blur text-sm shrink-0">
                <div className="text-slate-500 hidden sm:block">
                    Showing <span className="font-bold text-slate-700">{Math.min(paginatedDeals.length, deals.length)}</span> of <span className="font-bold text-slate-700">{deals.length}</span> deals
                </div>
