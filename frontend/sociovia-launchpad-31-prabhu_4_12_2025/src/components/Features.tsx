...
/**
 * SocioviaSecurity — static cards + Features-style layout
 * Changes: removed float/hover-lift animations, made cards static and equal-height,
 * removed group-hover dependencies and ensured consistent alignment.
 */

const fadeUp = {
  hidden: { opacity: 0, y: 12, scale: 0.998 },
  show: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.42, ease: "easeOut" } },
};
const stagger = { show: { transition: { staggerChildren: 0.06 } } };

function StaticCard({ children, className = "", motionProps = {} }: any) {
  // keep using motion for simple entrance; no hover/float interactions
  return (
    <motion.div
      {...motionProps}
      variants={fadeUp}
      className={`feature-gradient p-6 md:p-8 rounded-3xl shadow-soft animated-border transition-smooth h-full ${className}`}
    >
      {children}
    </motion.div>
  );
...
function IconAvatar({ Icon, gradient = "primary-gradient2" }: any) {
  return (
    <div className={`w-16 h-16 ${gradient} rounded-2xl flex items-center justify-center shadow-primary transition-smooth`}>
      <Icon className="w-8 h-8 text-primary-foreground" />
    </div>
  );
}

export default function SocioviaSecurity({ className = "" }: any) {
  const topHighlights = [
    { title: "Zero Data Retention", desc: "We enforce a strict zero-data-retention policy — your session data is processed and discarded.", icon: ShieldCheck, gradient: "primary-gradient2" },
    { title: "Enterprise-Grade Security", desc: "SAML, SSO and multi-layered protections with enterprise-ready controls.", icon: Lock, gradient: "accent-gradient" },
    { title: "Controlled Data Residency", desc: "Choose where your data lives with strict geographical residency controls.", icon: Globe, gradient: "primary-gradient2" },
  ];

  const gridItems = [
    { title: "You Own Your Data", desc: "Full exportability and clear ownership controls — always.", icon: Database, gradient: "primary-gradient2" },
    { title: "MFA & SSO", desc: "Multi-factor auth + federated single sign-on (SAML/OIDC).", icon: Users, gradient: "accent-gradient" },
    { title: "RBAC & Audit Logs", desc: "Fine-grained RBAC and immutable auditing for every change.", icon: Key, gradient: "primary-gradient2" },
    { title: "Configurable Retention", desc: "Policy-driven retention and safe deletion workflows.", icon: Zap, gradient: "accent-gradient" },
  ];
  ...
  return (
    <section className={`py-16 px-4 bg-background ${className}`}>
      <div className="max-w-6xl mx-auto space-y-12">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="mt-6 flex justify-center md:justify-start gap-4">
                  <a
                    href="/contact"
                    className="inline-flex items-center gap-3 px-5 py-3 rounded-xl btn-indigo shadow-primary"
                  ...
                  <a
                    href="/security"
                    className="inline-flex items-center gap-3 px-4 py-3 rounded-xl bg-white/06 border border-white/06 text-primary-foreground"
                  >
                    View Security Docs
                  </a>
                </div>
          ...
          {topHighlights.map((h, i) => (
            <StaticCard key={i}>
              <div className="flex items-start gap-4 h-full">
                <IconAvatar Icon={h.icon} gradient={h.gradient} />
        ...
        {/* SECURITY GRID — STATIC FEATURE CARDS */}
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp}>
          ...
          <motion.div variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6">
            {gridItems.map((g, i) => (
              <StaticCard key={i}>
                <div className="flex gap-4 items-start h-full">
                  <div className="flex-none">
                    <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${g.gradient ?? "feature-gradient"}`}>
        ...
        {/* ENTERPRISE TRUST + PRIVACY CARDS — STATIC */}
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} className="text-center">
          <StaticCard className="inline-block max-w-2xl">
            ...
            <div className="mt-4">
              <a href="mailto:security@sociovia.com" className="inline-flex items-center gap-3 px-5 py-3 rounded-xl btn-indigo shadow">
