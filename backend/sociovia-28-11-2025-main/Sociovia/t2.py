...
app.secret_key = os.getenv("SESSION_SECRET", app.config.get("SECRET_KEY", "dev-secret"))
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)
app.config["ALLOW_REQUEST_USER_ID_FALLBACK"] = True  # [fix this in after session validation]

# ---------- ENV-sensitive cookie settings ----------
# Make it easy to run locally (http) — set ENV=production in production
...
FB_API_VERSION = os.getenv("FB_API_VERSION", "v23.0")   # use v22.0 as default
...
def api_me():
    ...
    Auth order:
      1. session['user_id'] (normal)
      2. Authorization: Bearer <user_id> (dev/test ONLY if ALLOW_REQUEST_USER_ID_FALLBACK enabled)
      3. X-User-Id header (dev/test ONLY if ALLOW_REQUEST_USER_ID_FALLBACK enabled)

    Returns 401 when unauthenticated.
    """
    logger.debug("api_me: incoming session -> %s", dict(session))

    # 1) Primary: session-based
    user_id = session.get("user_id")
    if user_id:
        logger.debug("api_me: using session user_id=%s", user_id)
    else:
        # 2) Try Authorization: Bearer <user_id> (developer fallback)
        auth = request.headers.get("Authorization", "")
        user_id_from_auth = None
        if auth and auth.lower().startswith("bearer "):
            token = auth.split(None, 1)[1].strip()
            # treat numeric bearer token as user id in dev/testing only
            if token.isdigit():
                user_id_from_auth = int(token)

        # 3) Try X-User-Id header (developer fallback)
        user_id_from_header = None
        x_user = request.headers.get("X-User-Id") or request.headers.get("X-User-Id".lower())
        if x_user and str(x_user).isdigit():
            user_id_from_header = int(x_user)

        # Apply fallback only if permitted by config
        if not user_id:
            if current_app.config.get("ALLOW_REQUEST_USER_ID_FALLBACK", False):
                if user_id_from_auth:
                    user_id = user_id_from_auth
                    logger.warning("api_me: using ALLOW_REQUEST_USER_ID_FALLBACK via Authorization bearer (user_id=%s). Ensure this is only enabled in dev/test!", user_id)
                elif user_id_from_header:
                    user_id = user_id_from_header
                    logger.warning("api_me: using ALLOW_REQUEST_USER_ID_FALLBACK via X-User-Id header (user_id=%s). Ensure this is only enabled in dev/test!", user_id)
            else:
                logger.debug("api_me: no session and fallbacks disabled")
    ...
    try:
        ...
        logger.exception("api_me: error fetching user %s: %s", user_id, e)
        user = None
...
META_REDIRECT_URI = "https://sociovia-backend-362038465411.europe-west1.run.app/api/meta/callback"


LOGIN_FOR_BUSINESS_CONFIG_ID = os.getenv("LOGIN_FOR_BUSINESS_CONFIG_ID", "1957333988179911")

# This is a system user token you may have stored after minting (used AFTER connection)
SYSTEM_USER_TOKEN = os.getenv("SYSTEM_USER_TOKEN", None)
...
def appsecret_proof(token: str) -> str:
    """Compute appsecret_proof for a token using the app secret."""
    return hmac.new(
...
# ============================================================
# STEP 1: START BUSINESS LOGIN FOR CLIENT
# ============================================================
...
def meta_connect():
    ...
    "response_type": "code",
    "state": state,
    "config_id": LOGIN_FOR_BUSINESS_CONFIG_ID,  # <- required
    "scope": "public_profile,business_management,ads_management",
}
...
def meta_callback():
    ...
    if "error" in token_res:
        meta_log_error("TOKEN_ERROR", token_res["error"])
        return jsonify(token_res), 400

    client_user_token = token_res["access_token"]

    # -----------------------------
    # LOG THE CLIENT USER TOKEN (server logs only)
    # -----------------------------
    meta_log("CLIENT_USER_TOKEN", {"token": client_user_token})
    print("CLIENT_USER_TOKEN:", client_user_token)

    # Proceed to fetch business assets or system users
...
# ============================================================
# STEP 3: FETCH CLIENT BUSINESS + AD ACCOUNTS + SYSTEM USERS
# ============================================================

def fetch_business_assets(client_user_token, workspace_id, user_id):
    """
    Uses CLIENT USER TOKEN (must include business_management)
    Returns client_business_id, ad_accounts, and available system_users
    """

    # 1️⃣ Debug token to ensure we have business_management
    try:
        dbg = requests.get(
            f"{GRAPH}/debug_token",
            params={
                "input_token": client_user_token,
                "access_token": f"{FB_APP_ID}|{FB_APP_SECRET}",
            },
            timeout=10,
        ).json()
    except Exception as e:
        meta_log_error("DEBUG_TOKEN_FAIL", str(e))
        return jsonify({"success": False, "error": "debug_token_failed"}), 500

    meta_log("DEBUG_TOKEN", dbg.get("data", dbg))

    scopes = dbg.get("data", {}).get("scopes", []) or dbg.get("scopes", [])
    if "business_management" not in scopes:
        return jsonify({"success": False, "error": "missing_business_management"}), 400

    # 2️⃣ Get client business ID
    try:
        biz_res = requests.get(
            ...
            params={
                ...
                "appsecret_proof": appsecret_proof(client_user_token),
            },
            timeout=10,
        ).json()
    except Exception as e:
        meta_log_error("CLIENT_BUSINESS_FAIL", str(e))
        return jsonify({"success": False, "error": "client_business_lookup_failed"}), 500
    ...
    # 3️⃣ Get ad accounts client granted (use the client_user_token)
    try:
        ad_res = requests.get(
        ...
        meta_log_error("AD_ACCOUNTS_FAIL", str(e))
        return jsonify({"success": False, "error": "ad_accounts_fetch_failed"}), 500
    ...
    # 4️⃣ Get system users for this client business (requires business_management)
    try:
        su_res = requests.get(
            f"{GRAPH}/{client_business_id}/system_users",
            params={
                "access_token": client_user_token,
                "appsecret_proof": appsecret_proof(client_user_token),
            },
        ...
        meta_log_error("SYSTEM_USERS_FAIL", str(e))
        return jsonify({"success": False, "error": "system_users_fetch_failed"}), 500

    meta_log("SYSTEM_USERS", su_res)

    system_users = su_res.get("data", []) if isinstance(su_res, dict) else []
...
def meta_save():
    ...
    # Persist chosen ad account for the workspace (pseudo-code)
    # ad = AdAccount.query.filter_by(account_id=ad_account_id, workspace_id=workspace_id).first()
    # if not ad:
    #     ad = AdAccount(account_id=ad_account_id, workspace_id=workspace_id, user_id=user_id)
    #     db.session.add(ad)
    # db.session.commit()
...
def mint_system_user_token():
    ...
    expires_in_60_days = data.get("expires_in_60_days", False)
    ...
    params = {
        ...
        "appsecret_proof": appsecret_proof(user_admin_token),
    }

    if expires_in_60_days:
    ...
    # 🔐 STORE THIS TOKEN SECURELY (DB / Vault / Secrets Manager)
    # Example: save_system_user_token(workspace_id, new_token)
...
def test_system_token():
    ...
    if not SYSTEM_USER_TOKEN:
        return jsonify({"error": "no_system_user_token_configured"}), 400
...
@cross_origin(origins=ALLOWED_OAUTH_ORIGINS, supports_credentials=True)
def oauth_save_selection():
    ...
    selected_page_id = str(data.get("selected_page_id") or "").strip()
    selected_ad_account_id_raw = str(data.get("selected_ad_account_id") or "").strip()
    workspace_id = data.get("workspace_id")
    provided_token = data.get("access_token")
    provided_token_expires = data.get("token_expires_at")
    raw_user_id = data.get("user_id")

    if not selected_page_id:
        return jsonify({"success": False, "error": "missing_selected_page_id"}), 400
    if not selected_ad_account_id_raw:
        return jsonify({"success": False, "error": "missing_selected_ad_account_id"}), 400
    if workspace_id is None or str(workspace_id).strip() == "":
        return jsonify({"success": False, "error": "missing_workspace_id"}), 400

    # Keep workspace_id as string for SocialAccount (TEXT)
    workspace_id_str = str(workspace_id)

    selected_ad_account_id = normalize_account_id(selected_ad_account_id_raw)

    session_user = get_user_from_request(require=False)
    if session_user:
        user_id = session_user.id
        if raw_user_id and int(raw_user_id) != user_id:
            return jsonify({"success": False, "error": "user_id_mismatch"}), 403
    else:
        if request.headers.get("X-Internal-API-Key") != _cfg("INTERNAL_API_KEY"):
            return jsonify({"success": False, "error": "not_authenticated"}), 401
        try:
    ...
    try:
        # page (workspace-scoped) — use string workspace id for SocialAccount
        page = SocialAccount.query.filter_by(
            provider="facebook", provider_user_id=selected_page_id, workspace_id=workspace_id_str
        ).first()
        if page:
            if provided_token:
                page.access_token = provided_token
            page.ad_account_id = selected_ad_account_id
            page.user_id = user_id
            if not getattr(page, "workspace_id", None):
                page.workspace_id = workspace_id_str
            db.session.add(page)
            db.session.flush()
        else:
            page = SocialAccount(
                provider="facebook",
                provider_user_id=selected_page_id,
                account_name=data.get("page_name") or "",
        ...
        # ad account (workspace-scoped) — try integer workspace id for AdAccountModel if possible
        ws_int = _try_int(workspace_id_str)
        if ws_int is not None:
            ad = AdAccountModel.query.filter_by(account_id=selected_ad_account_id, workspace_id=ws_int).first()
        else:
            ad = AdAccountModel.query.filter_by(account_id=selected_ad_account_id, workspace_id=workspace_id_str).first()

        if ad:
            ad.user_id = user_id
            if provided_token:
                ad.access_token = provided_token
            if provided_token_expires:
                try:
                    ad.token_expires_at = parse_iso_datetime(provided_token_expires)
                except Exception:
                    pass
            ...
            ad = AdAccountModel(
                account_id=selected_ad_account_id,
                user_id=user_id,
                workspace_id=(ws_int if ws_int is not None else workspace_id_str),
                name=data.get("ad_account_name") or "",
                business_id=data.get("business_id") or None,
                access_token=provided_token,
                token_expires_at=(parse_iso_datetime(provided_token_expires) if provided_token_expires else None),
        ...
        # attach mapping (idempotent)
        try:
            attach_ad_to_page(db.session, page.id, ad.id, source="oauth_save_selection")
        except Exception:
            current_app.logger.exception("Failed to attach page->ad in oauth_save_selection for page %s ad %s", page.id, ad.id)
        ...
        response_payload = {
            "success": True,
            "connected": {
                "page": page.serialize() if hasattr(page, "serialize") else {
                    "provider_user_id": page.provider_user_id,
                    "ad_account_id": getattr(page, "ad_account_id", None),
                    "workspace_id": getattr(page, "workspace_id", None),
                    "user_id": getattr(page, "user_id", None),
                },
                "ad_account": {"account_id": ad.account_id, "id": ad.id, "workspace_id": ad.workspace_id},
            },
        }
...
def oauth_revoke():
    ...
    if not provider or not provider_user_id:
        return jsonify({"success": False, "error": "missing_provider_or_provider_user_id"}), 400

    sa = SocialAccount.query.filter_by(provider=provider, provider_user_id=str(provider_user_id), user_id=user.id).first()
    if not sa:
        return jsonify({"success": False, "error": "not_found"}), 404

    try:
        if sa.access_token:
            fb_api_version = _cfg("FB_API_VERSION", "v16.0")
        ...
        current_app.logger.exception("Failed to call fb revoke")
...
def fb_deauthorize():
    ...
    current_app.logger.info("FB deauthorize payload: %s", payload)
...
def fb_data_deletion():
    ...
    current_app.logger.info("FB data deletion request: %s", body)
    status_url = f"{_cfg('APP_BASE_URL', '').rstrip('/')}/data-deletion-status?request_id={int(datetime.now(timezone.utc).timestamp())}"
    return jsonify({"url": status_url}), 200


# ---------------- Facebook Meta helpers ----------------
def get_facebook_token_for_user(user_id, workspace_id=None):
    """
    Returns a workspace-scoped token if workspace_id provided, otherwise any token for the user.
    Uses string workspace id when querying SocialAccount (because social_accounts.workspace_id is TEXT).
    """
    if not user_id:
        return None
    try:
        if workspace_id is not None:
            # make sure we query SocialAccount with workspace_id as string
            sa = SocialAccount.query.filter_by(provider="facebook", user_id=user_id, workspace_id=str(workspace_id)).first()
            if sa and getattr(sa, "access_token", None):
                return sa.access_token
    except Exception:
        current_app.logger.exception("get_facebook_token_for_user (workspace-scoped) failed")

    sa = SocialAccount.query.filter_by(provider="facebook", user_id=user_id).first()
    if not sa:
        return None
    return getattr(sa, "access_token", None)
...
if not EXTERNAL_BASE_URL:
    EXTERNAL_BASE_URL = "https://sociovia-backend-362038465411.europe-west1.run.app"
...
def generate_from_image_endpoint():
    ...
    try:
        ...
        # ------------------------
        # Extract workspace context for better variation prompts
        # ------------------------
        ...
        if context_str:
            variations = [
                f"Create a professional marketing visual {context_str}. Use clean, modern design with strong visual hierarchy.",
                f"Generate an engaging social media graphic {context_str}. Make it attention-grabbing and shareable.",
                f"Design a brand-focused visual {context_str}. Emphasize trust, credibility, and professionalism."
            ]
        else:
            # Fallback to improved generic prompts
            variations = [
                "Create a professional marketing visual with clean, modern design.",
                "Generate an engaging social media graphic that is attention-grabbing.",
                "Design a creative, shareable visual for social media."
            ]
...
"""
Improved and robust Flask module for creating Facebook Ads (publish_v2).
- Resilient _graph_url builder
- Instrumented fb_post (no token logging)
- Image detection + upload helpers (Pillow + imghdr fallback)
- Resolve image hash from uploaded file / data-URIs / URLs
- Map-first targeting pipeline (accepts targeting, mapLocations)
- Interest keyword -> FB interest id resolution with simple TTL cache
- Timezone-aware timestamps
- Detailed logging for debugging

CONFIG (via env):
  FB_ACCESS_TOKEN, FB_AD_ACCOUNT_ID, FB_API_VERSION (eg 'v22.0'), GRAPH_BASE,
  FB_PAGE_ID, FB_PIXEL_ID, DEFAULT_IMAGE_HASH

Note: keep secrets out of logs (access_token is never logged).
"""
...
GRAPH_BASE = os.environ.get("GRAPH_BASE", "https://graph.facebook.com")

FB_PIXEL_ID = os.environ.get("FB_PIXEL_ID")
DEFAULT_IMAGE_HASH = os.environ.get("DEFAULT_IMAGE_HASH")

logger.info("GRAPH_BASE=%s FB_API_VERSION=%s FB_AD_ACCOUNT_ID=%s", GRAPH_BASE, FB_API_VERSION, FB_AD_ACCOUNT_ID)
...
def _acct_prefix(account_id: Optional[str]) -> Optional[str]:
    if not account_id:
        return None
    s = str(account_id)
    if s.startswith("act_"):
...
def _graph_url(path: str) -> str:
    """Build Graph URL robustly. If GRAPH_BASE already contains a version, use it.
    path should be the endpoint like 'search' or 'act_<id>/adimages'."""
    base = (GRAPH_BASE or "https://graph.facebook.com").rstrip('/')
    ver = (FB_API_VERSION or "").strip().lstrip('/')
    if re.search(r"/v\d+(\.\d+)?$", base):
        return f"{base}/{path.lstrip('/')}"
...
def fb_post(path, data=None, files=None, timeout=30):
    """POST helper which appends access_token and logs useful debug info (but not token).
    Returns parsed JSON or an error-like dict."""
    url = _graph_url(path)
    payload = dict(data or {})
    if FB_ACCESS_TOKEN:
        payload["access_token"] = FB_ACCESS_TOKEN
...
# ---------------------------
# Interest search cache (simple TTL cache)
# ---------------------------
...
def resolve_image_hash(body, image_file):
    ...
    if image_file and getattr(image_file, "filename", ""):
        try:
            ...
            if b:
                ...
                if h:
                    return h
                # fallback: if your infra supports upload_bytes_to_spaces, call it (not included here)
        except Exception:
            logger.exception("image_file upload failed")
    ...
    for u in (candidates or []):
        ...
        try:
            ...
            logger.exception("upload_image_by_url failed for candidate (outer)")
    ...
    try:
        ...
        def parse_request():
            ...
            if content_type.startswith("application/json"):
                ...
                body = request.form.to_dict() or {}
                image_file = request.files.get("image_file")
                # if creative came as stringified JSON
                if body.get("creative") and isinstance(body.get("creative"), str):
                    try:
                        body["creative"] = json.loads(body["creative"])
                    except Exception:
        ...
        # ----- names & simple fields -----
        now = datetime.now(timezone.utc)
        campaign_name = body.get("campaign_name") or f"Campaign {now.isoformat()}"
        adset_name = body.get("adset_name") or f"AdSet {now.isoformat()}"
        ad_name = body.get("ad_name") or f"Ad {now.isoformat()}"

        primary_text = (creative_in.get("primaryText") or creative_in.get("message") or "").strip()
        headline = (creative_in.get("headline") or "").strip()
        description = (creative_in.get("description") or "").strip()
        cta = body.get("cta") or "LEARN_MORE"
        link = (creative_in.get("url") or creative_in.get("link") or body.get("link") or "https://www.example.com").strip()
        ...
        mapped_obj = OBJECTIVE_MAP.get(requested_obj_upper)
        if not mapped_obj:
            if requested_obj_upper.startswith("OUTCOME_"):
                mapped_obj = requested_obj_upper
            else:
                logger.warning("Unknown objective %s, falling back to OUTCOME_TRAFFIC", requested_obj)
                mapped_obj = "OUTCOME_TRAFFIC"
        ...
        start_dt = (datetime.now(timezone.utc) + timedelta(days=start_in_days)).replace(microsecond=0)
        end_dt = (start_dt + timedelta(days=duration_days)).replace(microsecond=0)
        start_time = start_dt.isoformat().replace("+00:00", "-0000")
        end_time = end_dt.isoformat().replace("+00:00", "-0000")
        ...
        campaign_payload = {
            ...
            "objective": mapped_obj,
            "status": "PAUSED",
            "special_ad_categories": json.dumps(body.get("special_ad_categories") or []),
        }
        campaign_resp = fb_post(f"{_acct_prefix(FB_AD_ACCOUNT_ID)}/campaigns", data=campaign_payload)
        if isinstance(campaign_resp, dict) and campaign_resp.get("error"):
            logger.error("campaign create failed: %s", campaign_resp)
        ...
        # -------------------------
        # Build targeting: MAP-FIRST + interests
        # -------------------------
        raw_targeting = body.get("targeting")
        if isinstance(raw_targeting, str):
            try:
                targeting = json.loads(raw_targeting)
            except Exception:
                logger.warning("targeting string not valid JSON; ignoring", exc_info=True)
                targeting = {}
        elif isinstance(raw_targeting, dict):
            targeting = dict(raw_targeting)
        else:
            targeting = {}

        # map-based locations input
        map_locations = (
            body.get("mapLocations")
            or body.get("map_locations")
            or body.get("locations")
            or body.get("geo_locations")
            or None
        )
        ...
        def _normalize_and_dedupe_custom_locs(custom_locs):
            seen = set()
            out = []
            for loc in custom_locs:
                ...
                try:
                    ...
                    lng = float(loc.get("longitude") or loc.get("lng") or loc.get("lon"))
                except Exception:
                    continue
                radius = float(loc.get("radius") or loc.get("distance") or loc.get("radius_km") or 10)
                unit = loc.get("distance_unit") or loc.get("radius_unit") or "kilometer"
                key = (round(lat, 6), round(lng, 6), int(round(radius)))
                if key in seen:
                    continue
                seen.add(key)
                out.append({
                    ...
                    "radius": radius,
                    "distance_unit": unit,
                    "name": loc.get("name") or loc.get("label")
                })
            return out

        # If the client provided a dict with geo-like fields, merge carefully.
        if isinstance(map_locations, dict) and any(k in map_locations for k in ("countries", "cities", "regions", "zips", "custom_locations", "geo_locations")):
            inner_geo = map_locations.get("geo_locations") if isinstance(map_locations.get("geo_locations"), dict) else {}
            # copy top-level countries/cities/regions/zips/custom_locations if present
            if isinstance(map_locations.get("custom_locations"), list):
                geo["custom_locations"] = _normalize_and_dedupe_custom_locs(map_locations.get("custom_locations"))
            for k in ("countries", "cities", "regions", "zips"):
                if k in map_locations and map_locations.get(k):
                    geo[k] = map_locations.get(k)
            # merge inner geo_locations
            if inner_geo:
                if isinstance(inner_geo.get("custom_locations"), list):
                    geo.setdefault("custom_locations", [])
                    geo["custom_locations"].extend(_normalize_and_dedupe_custom_locs(inner_geo.get("custom_locations")))
                for k, v in inner_geo.items():
                    if k != "custom_locations":
                        geo[k] = v

        # If map_locations is a list of coordinate objects: build custom_locations
        elif isinstance(map_locations, list) and map_locations:
            custom_locs = []
            for loc in map_locations:
                ...
                if lat is None or lng is None:
                    continue
                radius = loc.get("radius") or loc.get("distance") or 10
                unit = loc.get("distance_unit") or loc.get("radius_unit") or "kilometer"
                try:
                    custom_locs.append({
                        "latitude": float(lat),
                    ...
                    continue
            if custom_locs:
                geo["custom_locations"] = _normalize_and_dedupe_custom_locs(custom_locs)

        # If we ended up with custom_locations, remove broad location keys to avoid overlapping-location errors.
        if geo.get("custom_locations"):
            if not geo.get("location_types"):
                geo["location_types"] = ["home", "recent"]
            for broad_key in ("countries", "cities", "regions", "zips"):
                geo.pop(broad_key, None)

        # ensure location_types default
        if not geo.get("location_types"):
            geo["location_types"] = ["home", "recent"]
        ...
        # -------------------------
        # Interests: keywords -> FB interest ids
        # -------------------------
        raw_interests = body.get("interest_keywords") or body.get("interest_ids") or body.get("interests") or []
        interest_objs = []
        if isinstance(raw_interests, str):
            raw_interests = [x.strip() for x in raw_interests.split(",") if x.strip()]

        if isinstance(raw_interests, list):
            for keyword in raw_interests:
                if not keyword:
                    continue
                if isinstance(keyword, dict) and keyword.get("id"):
                    interest_objs.append({"id": str(keyword.get("id"))})
                    continue
                # search FB for keyword and pick top result
                matches = search_facebook_interests(keyword, limit=3)
                if matches:
                    interest_objs.append({"id": str(matches[0]["id"])})

        if interest_objs:
            existing_flex = targeting.get("flexible_spec")
            if not isinstance(existing_flex, list) or not existing_flex:
                existing_flex = []
            if existing_flex:
                block = existing_flex[0]
                if not isinstance(block, dict):
                    block = {}
                current_interests = block.get("interests") or []
                if not isinstance(current_interests, list):
                    current_interests = []
                block["interests"] = current_interests + interest_objs
                existing_flex[0] = block
            else:
                existing_flex = [{"interests": interest_objs}]
            targeting["flexible_spec"] = existing_flex
        ...
        object_story_spec = {"page_id": FB_PAGE_ID, "link_data": link_data} if FB_PAGE_ID else {"link_data": link_data}
        creative_payload = {"name": f"Auto Creative {now.isoformat()}", "object_story_spec": json.dumps(object_story_spec)}
        creative_resp = fb_post(f"{_acct_prefix(FB_AD_ACCOUNT_ID)}/adcreatives", data=creative_payload)
        if isinstance(creative_resp, dict) and creative_resp.get("error"):
            logger.error("create adcreative failed: %s", creative_resp)
            return jsonify({"ok": False, "stage": "create_creative", "details": creative_resp}), 500
...
def generate_from_prodlink():
    ...
    try:
        ...
        short_desc = product.get("short_description") or product.get("description") or ""
        usps = (product.get("ad_campaign_ready") or {}).get("top_3_usps") or []
        usp_text = " | ".join(usps[:2]) if usps else ""

        # Marketing-focused creative brief (prioritize visual quality)
        marketing_brief = (
            f"""CREATIVE BRIEF: Generate a professional, scroll-stopping social media ad creative.

PRODUCT/BRAND: {title}
{f'TAGLINE: {one_line_tag}' if one_line_tag else ''}
{f'DESCRIPTION: {short_desc[:200]}' if short_desc else ''}
{f'KEY BENEFITS: {usp_text}' if usp_text else ''}

VISUAL DESIGN REQUIREMENTS (CRITICAL - FOLLOW THESE):
1. Create a MODERN, PREMIUM marketing visual that looks like a professionally designed social media ad
2. Use BOLD, READABLE typography - ensure all text is clearly legible with high contrast
3. Include a COMPELLING HEADLINE that grabs attention immediately
4. Design should feel PROFESSIONAL, TRUSTWORTHY, and on-brand
5. Use VIBRANT colors with dynamic composition and clean layout
6. Add a CLEAR call-to-action button or text element
7. The image should look like an ad from a Fortune 500 company - polished and high-quality

STYLE GUIDELINES:
- Clean, modern design with strong visual hierarchy
- Professional gradient backgrounds OR solid brand colors
- Bold, sans-serif fonts for headlines (like Montserrat, Poppins, or Inter style)
- Adequate white space and breathing room
- Text overlays should have backgrounds/shadows for readability
- Suitable for Instagram, Facebook, or LinkedIn ads

DO NOT:
- Use cluttered or busy designs
- Make text hard to read
- Create abstract art - this should be a MARKETING POST
- Include placeholder text like "Headline" or "Lorem ipsum"
- Generate low-quality or blurry images

REFERENCE IMAGES: Use the provided website snapshots to understand the brand/product context and visual style, but create a NEW, original marketing creative - not a screenshot edit.

OUTPUT: Generate ONE high-quality marketing creative image.
After the image, provide a brief JSON with: headline, body_text, cta (call-to-action text).
"""
        )
        if one_line_tag:
            marketing_brief += f"\nSuggested tagline to incorporate: {one_line_tag}\n"
        if user_prompt:
...
def facebook_adpreviews():
    """
    Create temporary adcreative (if needed), enforce & attach a single STATIC lead form,
    and fetch creative previews. Returns previews + debug info for lead-form verification.

    Accepts:
      - creative_id OR creative (blob)
      - static_lead_form.id OR env STATIC_LEAD_FORM_ID (preferred)
      - ad_formats, preview_options, api_version
    """
    try:
        body = request.get_json(silent=True) or {}
        ...
        attached_lead_form_id = None
        lead_form_debug: Dict[str, Any] = {}

        creative_blob = body.get("creative") or body.get("creative_for_preview") or None
        object_story_spec = None

        # Build basic object_story_spec from creative_blob when necessary
        if creative_blob and not creative_id:
            try:
                ...
                if oss and isinstance(oss, dict):
                    ...
                    # Call to action (allow passing a CTA from creative blob, but we will override for lead form)
                    ...
                    # Resolve image_hash (best effort)
                    image_hash = None
                    ...
                    if not image_hash and (image_url or creative_blob.get("image")) and fb_ad_account:
                        try:
                            ...
                            if "get_image_hash_from_payload" in globals() and callable(globals()["get_image_hash_from_payload"]):
                                image_hash = get_image_hash_from_payload(creative_blob or {}, src, api_version, fb_ad_account, access_token)
                                if not image_hash:
                                    current_app.logger.warning("get_image_hash_from_payload returned no image_hash.")
                        except Exception as e:
        ...
        # -------------------------
        # Enforced static lead form (hardcoded with fallback) — verify existence and use only this
        # -------------------------
        def _verify_lead_form_exists(form_id: str, api_ver: str, page_token: str) -> Tuple[bool, Dict[str, Any]]:
            debug: Dict[str, Any] = {"ok": False, "status": None, "response_json": None, "response_text": None}
            if not form_id:
                debug["response_text"] = "no form_id provided"
                return False, debug

            verify_url = f"https://graph.facebook.com/{api_ver}/{form_id}"
            params = {"access_token": page_token}
            try:
                r = requests.get(verify_url, params=params, timeout=10)
                debug["status"] = getattr(r, "status_code", None)
                try:
                    jr = r.json()
                    debug["response_json"] = jr
                    if isinstance(jr, dict) and (jr.get("id") == str(form_id) or jr.get("id")):
                        debug["ok"] = True
                        return True, debug
                    else:
                        debug["response_text"] = json.dumps(jr, ensure_ascii=False)
                        return False, debug
                except ValueError:
                    debug["response_text"] = r.text
                    return False, debug
            except requests.RequestException as e:
                debug["response_text"] = f"requests exception: {str(e)}"
                current_app.logger.exception("verify_lead_form_exists request failed: %s", e)
                return False, debug

        # Priority: env STATIC_LEAD_FORM_ID (preferred) OR body.static_lead_form.id (fallback) OR built-in default
        lead_form_id = None
        attached_lead_form_id = None
        page_token = os.getenv("FB_PAGE_ACCESS_TOKEN") or os.getenv("FB_PAGE_TOKEN") or access_token

        env_static = os.getenv("STATIC_LEAD_FORM_ID")
        body_static = (body.get("static_lead_form") or body.get("staticLeadForm") or {})
        body_static_id = body_static.get("id") if isinstance(body_static, dict) else None

        chosen_form_id = None
        if env_static:
            chosen_form_id = str(env_static)
            current_app.logger.info("STATIC_LEAD_FORM_ID found in environment: %s", chosen_form_id)
        elif body_static_id:
            chosen_form_id = str(body_static_id)
            current_app.logger.info("static_lead_form.id found in body: %s", chosen_form_id)
        else:
            # Fallback to built-in default (hardcoded)
            chosen_form_id = DEFAULT_STATIC_LEAD_FORM_ID
            current_app.logger.info("No STATIC_LEAD_FORM_ID in env/body — falling back to DEFAULT_STATIC_LEAD_FORM_ID: %s", chosen_form_id)

        # Verify it actually exists (and we can access it)
        exists, verify_debug = _verify_lead_form_exists(chosen_form_id, api_version, page_token)
        lead_form_debug = _safe_serialize(verify_debug)
        if not exists:
            current_app.logger.warning("STATIC_LEAD_FORM_ID verification failed: %s", lead_form_debug)
            return jsonify({"ok": False, "error": "static_lead_form_invalid", "details": lead_form_debug}), 400

        # All good — use this static form id and attach it to previews
        lead_form_id = chosen_form_id
        attached_lead_form_id = chosen_form_id
        current_app.logger.info("Using verified static lead form id for preview: %s", lead_form_id)

        # Attach lead form to object_story_spec if available — use an allowed CTA type and include only lead_gen_form_id in value
        try:
            if (lead_form_id or attached_lead_form_id) and isinstance(object_story_spec, dict):
                link = object_story_spec.get("link_data") or {}
                # Choose CTA type via env DEFAULT_CTA_TYPE, fallback to DEFAULT_CTA_TYPE variable above
                cta_type = os.getenv("DEFAULT_CTA_TYPE") or DEFAULT_CTA_TYPE or "LEARN_MORE"
                cta_type = str(cta_type).upper()
                # IMPORTANT: only send lead_gen_form_id here — Facebook rejects 'form_id'
                cta = {
                    "type": cta_type,
                    "value": {
                        "lead_gen_form_id": str(lead_form_id or attached_lead_form_id)
                    }
                }
                link["call_to_action"] = cta
                object_story_spec["link_data"] = link
                current_app.logger.info("Attached lead form %s to object_story_spec for preview with CTA type %s.", (lead_form_id or attached_lead_form_id), cta_type)
        except Exception:
            current_app.logger.exception("Failed to attach lead form to object_story_spec")

        # Create temporary creative if needed
        if not creative_id and object_story_spec:
            if fb_ad_account:
                try:
                    creative_payload = {
                    ...
                    if isinstance(created, dict) and created.get("id"):
                        ...
                        current_app.logger.warning("Adcreative creation didn't return id; response=%s", _safe_serialize(created))

                    if isinstance(created, dict) and created.get("error"):
                        return jsonify({"ok": False, "error": "adcreative_create_failed", "details": created}), 400
                except Exception:
                    current_app.logger.exception("Failed to create adcreative for preview")
            else:
        ...
        for fmt in ad_formats:
            ...
            try:
                ...
                current_app.logger.info("FB GET -> url=%s params=%s", preview_url, {k: ("<REDACTED>" if k == "access_token" else v) for k, v in params.items()})
                r.raise_for_status()
            except requests.RequestException as e:
                ...
                last_error = {"format": fmt, "error": str(e), "status": status, "response_text": (text[:2000] if text else None)}
                previews.append({"format": fmt, "ok": False, "error": str(e), "status": status, "response_text": (text[:2000] if text else None)})
                continue
            ...
            try:
                ...
                current_app.logger.exception("Error extracting preview_html")

            previews.append(entry)
            any_success = True

        if not any_success:
            payload = {"ok": False, "error": "all_preview_requests_failed", "detail": _safe_serialize(last_error), "previews": _safe_serialize(previews)}
            if lead_form_debug:
                payload["lead_form_debug"] = _safe_serialize(lead_form_debug)
            return jsonify(payload), 502

        # quick top-level preview URL for convenience (first iframe_src)
        preview_url = None
        if previews:
            first_p = previews[0]
            preview_url = first_p.get("iframe_src") or (
                first_p.get("preview_html") and re.search(r'<iframe[^>]+src=["\']([^"\']+)["\']', first_p.get("preview_html") or "", re.I)
                and re.search(r'<iframe[^>]+src=["\']([^"\']+)["\']', first_p.get("preview_html") or "", re.I).group(1)
            )
        if preview_url:
            current_app.logger.info("Preview URL for quick open: %s", preview_url)

        resp = {"ok": True, "creative_id": creative_id, "previews": previews, "preview_url": preview_url}
        if created_creative_id:
            resp["created_creative_id"] = created_creative_id
        if attached_lead_form_id:
            resp["attached_lead_form_id"] = attached_lead_form_id
        if lead_form_debug:
            resp["lead_form_debug"] = _safe_serialize(lead_form_debug)
...
def generate_copy():
    ...
    if workspace is None and workspace_id:
        try:
            ws_url = f"https://sociovia-backend-362038465411.europe-west1.run.app/api/workspace?workspace_id={int(workspace_id)}"
...
# ---------- GENERATE TEXT ----------
def _generate_text_from_prompt(prompt, model_id=None, temperature=0.2, max_output_tokens=512):
    client = GENAI_CLIENT
