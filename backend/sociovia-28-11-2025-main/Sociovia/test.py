...
FRONTEND_ORIGINS = [
    ...
    "https://sociovia.com",
    "https://rlcslwgm-8080.inc1.devtunnels.ms",
    "http://127.0.0.1:8080",
    "http://localhost:3000",
    "http://localhost:8080",
]
IS_LOCAL = os.getenv("ENV", "dev") != "production"
...
app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {
    "pool_size": 5,        # max open connections per process
...
# ---------- DB session cleanup & error handling ----------
from sqlalchemy.exc import OperationalError

@app.teardown_appcontext
def shutdown_session(exception=None):
    """Always release DB session at the end of the request."""
    try:
        db.session.remove()
    except Exception:
        pass
...
def handle_db_operational_error(e):
    """
    When Postgres runs out of connection slots or drops connections,
    return a clean 503 instead of a giant stack trace.
    """
    current_app.logger.exception("Database operational error: %s", e)
    return jsonify({
...
FB_API_VERSION = os.getenv("FB_API_VERSION", "v22.0")   # use v22.0 as default
...
# initialize server-side session support
...
"""
# Allow ALL origins with credentials - dynamically reflect the origin
CORS(
    app,
    resources={r"/*": {"origins": "*"}},
    supports_credentials=True,
    allow_headers=["Content-Type", "Authorization", "X-User-Id", "X-Workspace-ID"],
    expose_headers=["Content-Type"],
...
def add_cors_headers(resp):
    # Dynamically set Access-Control-Allow-Origin to the requesting origin
    origin = request.headers.get("Origin")
    if origin:
        resp.headers["Access-Control-Allow-Origin"] = origin
        resp.headers["Access-Control-Allow-Credentials"] = "true"
        resp.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-Requested-With, X-User-Id, X-User-Email"
        resp.headers["Access-Control-Allow-Methods"] = "GET, POST, PUT, DELETE, OPTIONS, PATCH"
    resp.headers["Vary"] = "Origin"
    return resp

# small debug helper to log auth/cookie presence for incoming requests (dev only)
@app.before_request
def log_auth_debug():
    try:
        auth_hdr = request.headers.get("Authorization", None)
        if not IS_PROD:
            current_app.logger.debug("Request %s %s Authorization=%s Cookie=%s session_keys=%s",
                                     request.method, request.path,
                                     auth_hdr if auth_hdr is None else "<present>",
                                     request.headers.get("Cookie", "<none>"),
                                     list(session.keys()))
        else:
            current_app.logger.debug("Request %s %s Authorization present=%s session_keys=%s",
                                     request.method, request.path,
                                     bool(auth_hdr),
                                     list(session.keys()))
    except Exception:
...
# attach singletons for crm package to access
app.db = db
app.ma = ma

# ---------------- Register CRM blueprint and optionally seed demo data --------------
# Must be executed inside the application context because create_crm_blueprint()
# and seed_demo_data() use current_app / app.db / app.ma.
with app.app_context():
    try:
        crm_bp = create_crm_blueprint()
        # optionally apply auth gating here (if you have apply_auth_to_blueprint)
        app.register_blueprint(crm_bp)
        logger.info("CRM blueprint registered at /api")
    except Exception as e:
        logger.exception("Failed to create/register CRM blueprint inside app context: %s", e)

    # Seed demo data (dev only). Import inside app_context so it can access current_app
    try:
        from SocioviaCrm.seed import seed_demo_data
        seed_demo_data()
        logger.info("Seeded CRM demo data")
    except ImportError as e:
        logger.debug("CRM seed import failed (maybe file / function name mismatch): %s", e)
    except Exception as e:
        logger.debug("CRM seed skipped or failed: %s", e)

# ---------------- Analytics module init & register (inside app_context) -----------
with app.app_context():
    try:
        from SocioviaCrm.analytics import init_analytics_models, start_scheduler, bp as analytics_bp
        init_analytics_models()
        app.register_blueprint(analytics_bp)
        logger.info("Analytics blueprint registered at /api/analytics")
    except Exception as e:
        logger.exception("Failed to init analytics module: %s", e)

# ---------------- Meta / GetWorkspaceData blueprint registration ----------------
# IMPORTANT: import this inside app.app_context() (the module must not access current_app at import time)
with app.app_context():
    try:
        # do not import this at top level — import inside context to avoid 'working outside app context'
        from MetaHelpers.GetWorkspaceData import bp as meta_bp
        app.register_blueprint(meta_bp)
        logger.info("Meta/GetWorkspaceData blueprint registered at /api/meta")
    except Exception as e:
        logger.exception("Failed to register Meta/GetWorkspaceData blueprint: %s", e)

# ---------------- WhatsApp Phase 1 Blueprint Registration ----------------
# Clean, isolated WhatsApp module - NO workspace dependencies
with app.app_context():
    try:
        from whatsapp import whatsapp_bp
        app.register_blueprint(whatsapp_bp, url_prefix='/api/whatsapp')
        logger.info("WhatsApp Phase 1 blueprint registered at /api/whatsapp")
    except Exception as e:
        logger.exception("Failed to register WhatsApp blueprint: %s", e)

# ---------------- WhatsApp Automation Blueprint Registration ----------------
with app.app_context():
    try:
        from whatsapp import automation_bp
        app.register_blueprint(automation_bp, url_prefix='/api/whatsapp')
        logger.info("WhatsApp Automation blueprint registered at /api/whatsapp")
    except Exception as e:
        logger.exception("Failed to register WhatsApp Automation blueprint: %s", e)

# ---------------- WhatsApp AI Chatbot Blueprint Registration ----------------
with app.app_context():
    try:
        from whatsapp import ai_bp
        app.register_blueprint(ai_bp)  # url_prefix already set in blueprint
        logger.info("WhatsApp AI Chatbot blueprint registered at /api/whatsapp/accounts")
    except Exception as e:
        logger.exception("Failed to register WhatsApp AI Chatbot blueprint: %s", e)

# ---------------- WhatsApp FAQ Blueprint Registration ----------------
with app.app_context():
    try:
        from whatsapp import faq_bp
        app.register_blueprint(faq_bp)  # url_prefix already set in blueprint
        logger.info("WhatsApp FAQ blueprint registered at /api/whatsapp/accounts")
    except Exception as e:
        logger.exception("Failed to register WhatsApp FAQ blueprint: %s", e)

# ---------------- WhatsApp Template Acceleration Blueprint ----------------
with app.app_context():
    try:
        from whatsapp.template_routes import template_bp
        app.register_blueprint(template_bp)  # url_prefix already set in blueprint
        logger.info("Template Acceleration blueprint registered at /api/whatsapp/templates")
    except Exception as e:
        logger.exception("Failed to register Template Acceleration blueprint: %s", e)

# ---------------- WhatsApp Flows Blueprint ----------------
with app.app_context():
    try:
        from whatsapp.flow_routes import flow_bp
        app.register_blueprint(flow_bp)  # url_prefix set in blueprint: /api/whatsapp/flows
        logger.info("WhatsApp Flows blueprint registered at /api/whatsapp/flows")
    except Exception as e:
        logger.exception("Failed to register WhatsApp Flows blueprint: %s", e)

# ---------------- WhatsApp Flow Endpoint (Dynamic Flows) ----------------
with app.app_context():
    try:
        from whatsapp.flow_endpoint import flow_endpoint_bp
        app.register_blueprint(flow_endpoint_bp)  # Adds /endpoint, /keys/generate, /health
        logger.info("WhatsApp Flow Endpoint blueprint registered (dynamic flows)")
    except Exception as e:
        logger.exception("Failed to register Flow Endpoint blueprint: %s", e)

# ---------------- WhatsApp Flow Testing (Preview, Test Data, Publish Check) ----------------
with app.app_context():
    try:
        from whatsapp.flow_testing import flow_testing_bp
        app.register_blueprint(flow_testing_bp)  # Adds /preview, /test-data, /publish-check, /simulate
        logger.info("WhatsApp Flow Testing blueprint registered")
    except Exception as e:
        logger.exception("Failed to register Flow Testing blueprint: %s", e)

# ---------------- WhatsApp Trigger Blueprint ----------------
with app.app_context():
    try:
        from whatsapp import trigger_bp
        app.register_blueprint(trigger_bp)
        logger.info("WhatsApp Trigger blueprint registered")
    except Exception as e:
        logger.exception("Failed to register WhatsApp Trigger blueprint: %s", e)

# ---------------- WhatsApp Drip Blueprint ----------------
with app.app_context():
    try:
        from whatsapp import drip_bp
        app.register_blueprint(drip_bp)
        logger.info("WhatsApp Drip blueprint registered")
    except Exception as e:
        logger.exception("Failed to register WhatsApp Drip blueprint: %s", e)

# ---------------- WhatsApp Interactive Automation Blueprint ----------------
with app.app_context():
    try:
        from whatsapp import interactive_automation_bp
        app.register_blueprint(interactive_automation_bp, url_prefix='/api/whatsapp')
        logger.info("WhatsApp Interactive Automation blueprint registered at /api/whatsapp/interactive-automations")
    except Exception as e:
        logger.exception("Failed to register WhatsApp Interactive Automation blueprint: %s", e)

# ---------------- CTWA (Click-to-WhatsApp Ads) Blueprint Registration ----------------
with app.app_context():
    try:
        from ctwa import ctwa_bp
        app.register_blueprint(ctwa_bp)  # url_prefix already set in blueprint
        logger.info("CTWA blueprint registered at /api/ctwa")
    except Exception as e:
        logger.exception("Failed to register CTWA blueprint: %s", e)

# ---------------- Start scheduler ----------------
# start_scheduler reads app.config for ANALYTICS_FETCH_HOURS; pass app to it
try:
    from SocioviaCrm.analytics import start_scheduler
    start_scheduler(app)
except Exception as e:
    logger.exception("Failed to start analytics scheduler: %s", e)
...
def api_login():
    ...
    # Include status for fallback when cookies don't work (incognito/mobile)
...
def api_me():
    # Try multiple auth methods for cross-origin support
    user_id = session.get("user_id")
    logger.debug("api_me: incoming session -> %s", dict(session))

    # Fallback to X-User-Id header if no session (cross-origin cookie issue)
    if not user_id:
    ...
    if not user:
        # invalid user id — clear session if present
...
def api_workspace_setup_create():
    ...
    try:
        ...
        # --- Parse address fields (support both nested object and individual fields) ---
        address_obj = None
        address_raw = form.get("address")
        if address_raw:
            try:
                address_obj = json.loads(address_raw) if isinstance(address_raw, str) else address_raw
            except (json.JSONDecodeError, TypeError):
                address_obj = None
        
        # Extract individual address fields - prefer nested object, fallback to individual fields
        if address_obj and isinstance(address_obj, dict):
            address_line = (address_obj.get("address_line") or "").strip()
            city = (address_obj.get("city") or "").strip()
            district = (address_obj.get("district") or "").strip()
            pin_code = (address_obj.get("pin_code") or "").strip()
            country = (address_obj.get("country") or "India").strip()
        else:
            address_line = (form.get("address_line") or "").strip()
            city = (form.get("city") or "").strip()
            district = (form.get("district") or "").strip()
            pin_code = (form.get("pin_code") or "").strip()
            country = (form.get("country") or "India").strip()
        
        # Legacy fallback - parse registered_address if new fields are empty
        registered_address_raw = (form.get("registered_address") or "").strip()
        if not address_line and registered_address_raw:
            parts = [p.strip() for p in registered_address_raw.split(",")]
            address_line = parts[0] if len(parts) > 0 else ""
            city = parts[1] if len(parts) > 1 else ""
            district = parts[2] if len(parts) > 2 else ""
            pin_code = parts[3] if len(parts) > 3 else ""
            country = parts[4] if len(parts) > 4 else "India"
        
        # Build combined registered_address for backward compatibility
        address_parts = [p for p in [address_line, city, district, pin_code, country] if p]
        registered_address = ", ".join(address_parts) if address_parts else ""
        ...
        if not address_line or len(address_line) < 5:
            errors.append("address_line_required")
        if not city or len(city) < 2:
            errors.append("city_required")
        if not pin_code or len(pin_code) < 4:
            errors.append("pin_code_required")
        ...
        workspace.address_line = address_line
        workspace.city = city
        workspace.district = district or None
        workspace.pin_code = pin_code
        workspace.country = country or 'India'
        # Keep registered_address for backward compatibility
...
class Generation(db.Model):
    ...
    session_id = db.Column(db.String(64), nullable=True, index=True)  # For grouping prompts in same chat session
...
def api_workspace(workspace_id):
    ...
    try:
        ...
        if request.method == 'GET':
            # Fetch generations (conversations) and filter out soft-deleted
            all_generations = Generation.query.filter_by(workspace_id=workspace_id).order_by(Generation.created_at.desc()).all()
            generations_data = []
            for g in all_generations:
                if is_conversation_deleted(g):
                    continue
                # Get the display title (respects renames)
                display_title = get_conversation_title(g)
                generations_data.append({
                    "id": g.id,
                    "prompt": g.prompt,
                    "title": display_title,  # Use renamed title if available
                    "response": g.response,
                    "created_at": g.created_at.isoformat() if g.created_at else None
                })
            logger.info(f"Fetched {len(generations_data)} active generations (filtered out deleted) for workspace {workspace_id}")
            ...
            return jsonify({
                ...
                "workspace": {
                    ...
                    "business_type": workspace.business_type,
                    "address": {
                        "address_line": getattr(workspace, 'address_line', None),
                        "city": getattr(workspace, 'city', None),
                        "district": getattr(workspace, 'district', None),
                        "pin_code": getattr(workspace, 'pin_code', None),
                        "country": getattr(workspace, 'country', 'India')
                    },
                    "registered_address": workspace.registered_address,
                    "b2b_b2c": workspace.b2b_b2c,
                    "industry": workspace.industry,
        ...
        fields = [
            ...
            'address_line', 'city', 'district', 'pin_code', 'country'
        ]
        for field in fields:
...
# ---------------------------------------------------------------------
# Helper to check if a generation/conversation is soft-deleted
# ---------------------------------------------------------------------
def is_conversation_deleted(gen_row):
    """
    Returns True if the conversation row is marked as deleted.
    Checks dedicated column (if exists) or the JSON payload.
    """
    # 1. Check for dedicated DB column (future proofing)
    if hasattr(gen_row, 'deleted') and gen_row.deleted:
        return True
        
    # 2. Check inside the JSON response blob
    if gen_row.response:
        try:
            data = json.loads(gen_row.response)
            if isinstance(data, dict):
                # Check direct _deleted flag
                if data.get('_deleted') is True:
                    return True
        except Exception:
            pass
...
# ---------------------------------------------------------------------
# Helper to get display title (respects renames)
# ---------------------------------------------------------------------
def get_conversation_title(gen_row):
    """
    Returns the display title for a conversation.
    If renamed, returns the new title. Otherwise returns the prompt.
    """
    # Check for renamed title in response JSON
    if gen_row.response:
        try:
            data = json.loads(gen_row.response)
            if isinstance(data, dict):
                renamed_title = data.get('_renamed_to')
                if renamed_title:
...
def api_generations():
    ...
    try:
        ...
        # Fetch all from DB
        all_generations = Generation.query.filter(Generation.workspace_id == str(workspace_id)).order_by(Generation.created_at.desc()).all()
        logger.info(f"Fetched {len(all_generations)} total generations from DB for workspace {workspace_id}")

        # Filter out soft-deleted items in Python
        generations_data = []
        for g in all_generations:
            deleted = is_conversation_deleted(g)
            logger.debug(f"Generation {g.id}: is_deleted={deleted}, response_preview={g.response[:100] if g.response else 'empty'}")
            if deleted:
                logger.info(f"Filtering out deleted generation {g.id}")
                continue
            ...
            generations_data.append({
                ...
                "title": display_title,  # Use renamed title if available
                "response": g.response,
                "session_id": getattr(g, 'session_id', None),  # For grouping prompts in same chat session
                "created_at": g.created_at.isoformat() if g.created_at else None
            })

        logger.info(f"Returning {len(generations_data)} active generations (filtered out {len(all_generations) - len(generations_data)} deleted) for workspace {workspace_id}")
...
def api_generations_me():
    ...
    try:
        ...
        all_generations = Generation.query.filter_by(user_id=user.id, workspace_id=None).order_by(Generation.created_at.desc()).all()
        ...
        logger.info(f"Fetched {len(generations_data)} active generations (filtered out deleted) for user {user.id}")
...
def enhance_prompt():
    """
    Enhance a user's prompt/text using Gemini LLM.
    Accepts: original_text, base_prompt, hint, or prompt field.
    Returns: enhanced_text with improved clarity, creativity, and professionalism.
    """
    try:
        data = request.get_json() or {}
        ...
        # Extract just the user's text if it came through hint with prefix
        if original_text.lower().startswith('enhance and improve this text professionally:'):
        ...
        if not client:
            logger.error("[enhance_prompt] GENAI_CLIENT not initialized")
            return jsonify({"success": False, "error": "AI service not available"}), 503
        
        # Build enhancement prompt
        enhancement_prompt = f"""You are an expert copywriter and marketing specialist. Your task is to enhance and improve the following text/prompt while keeping its core meaning and intent intact.

Guidelines:
- Make it more compelling, clear, and professional
- Improve word choice and flow
- Add creative flair where appropriate
- Keep the same general length (don't make it excessively longer)
- Maintain the original tone (casual, formal, etc.) unless it's unclear
- If it's an ad/marketing copy, make it more persuasive
- If it's a creative prompt, make it more vivid and descriptive
- Return ONLY the enhanced text, no explanations or prefixes
...
Enhanced version:"""
        ...
        model_id = globals().get("TEXT_MODEL") or "gemini-2.0-flash"
        logger.info(f"[enhance_prompt] Enhancing text with model={model_id}")
        ...
        if not enhanced_text:
            logger.warning("[enhance_prompt] No enhanced text generated, returning original")
            enhanced_text = original_text
        
        logger.info(f"[enhance_prompt] Successfully enhanced text ({len(original_text)} -> {len(enhanced_text)} chars)")
        ...
        logger.exception("[enhance_prompt] Error enhancing text")
        return jsonify({
            "success": False, 
...
# Define SavedImage model with expiration support
class SavedImage(db.Model):
    ...
    workspace_id = db.Column(db.String(128), nullable=False, index=True)
    url = db.Column(db.String(512), nullable=False)
    filename = db.Column(db.String(256))
    original_url = db.Column(db.String(512))  # Original source URL
    saved_key = db.Column(db.String(256))  # S3/Spaces key
    
    # Expiration fields
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    expires_at = db.Column(db.DateTime, nullable=True, index=True)  # NULL means check is_permanent
    is_permanent = db.Column(db.Boolean, default=False)  # If True, never expires
    expiry_notified = db.Column(db.Boolean, default=False)  # For email notifications
    
    def __repr__(self):
        return f'<SavedImage {self.id}>'
    
    def to_dict(self):
        """Convert to dictionary for JSON serialization"""
        now = datetime.now(timezone.utc)
        days_remaining = None
        if self.expires_at and not self.is_permanent:
...
def api_workspace_get():
    ...
    try:
        ...
        # Debug logging
        logger.debug("api_workspace_get: user=%s, q_user_id=%s, session=%s, headers.X-User-Id=%s",
                     user, q_user_id, dict(session), request.headers.get("X-User-Id"))
        ...
        if q_user_id_int is not None:
            ...
            return jsonify({
                ...
                "message": "Please provide user_id query param, X-User-Id header, or log in to create a session"
            }), 400
        ...
        if q_workspace_id_int is not None:
            ...
            return jsonify({
                ...
                "workspace": {
                    ...
                    "user_id": workspace.user_id,
                    "business_name": workspace.business_name,
                    "business_type": getattr(workspace, "business_type", None),
                    "address": {
                        "address_line": getattr(workspace, "address_line", None),
                        "city": getattr(workspace, "city", None),
                        "district": getattr(workspace, "district", None),
                    ...
                    "registered_address": getattr(workspace, "registered_address", None),
                    "b2b_b2c": getattr(workspace, "b2b_b2c", None),
        ...
        for w in workspaces:
            ...
            out.append({
                ...
                "business_type": getattr(w, "business_type", None),
                "address": {
                    "address_line": getattr(w, "address_line", None),
                    "city": getattr(w, "city", None),
                    "district": getattr(w, "district", None),
                    "pin_code": getattr(w, "pin_code", None),
                    "country": getattr(w, "country", "India")
                },
                "registered_address": getattr(w, "registered_address", None),
                "b2b_b2c": getattr(w, "b2b_b2c", None),
...
def after_request(response):
    response.headers.add("Access-Control-Allow-Origin", "https://sociovia.com")
...
def get_workspace_me():
    ...
    result = {
        ...
        "address": {
            "address_line": g("address_line") or "",
            "city": g("city") or "",
            "district": g("district") or "",
            "pin_code": g("pin_code") or "",
...
# server/routes_oauth.py
"""
Business-only OAuth for Sociovia.ai
----------------------------------
• Facebook Login for Business (config_id)
• Ad-Account centric
• No Page API usage
• Business-safe & App-Review compliant
"""


"""
Meta (Facebook) Login for Business — Sociovia.ai
------------------------------------------------
FLOW:
1. User clicks "Connect Meta Ads"
2. Facebook Login for Business (Business chooses assets)
3. Exchange code → USER token (TEMP)
4. Fetch businesses + ad accounts
5. Store ONLY IDs
6. Use SYSTEM USER TOKEN for ads execution
"""
...
META_REDIRECT_URI = "https://rlcslwgm-5000.inc1.devtunnels.ms/api/meta/callback"

LOGIN_FOR_BUSINESS_CONFIG_ID = os.getenv("LOGIN_FOR_BUSINESS_CONFIG_ID")
...
def meta_connect():
    """
    Redirect user to Facebook Login for Business
    """
    ...
    meta_log("CONNECT INIT", state_payload)
    meta_log("REDIRECT URL", auth_url)
...
def meta_callback():
    ...
    if error:
        meta_log_error("META ERROR", error)
    ...
    meta_log("FB ME RESPONSE", me_res)

    fb_user_id = me_res.get("id")
    account_name = me_res.get("name")

    if not fb_user_id:
        return jsonify({
            "success": False,
            "error": "facebook_user_id_missing"
        }), 400

    # --------------------------------------------------------
    # UPSERT SOCIAL ACCOUNT ✅ FIXED
    # --------------------------------------------------------
    social = (
        SocialAccount.query
        .filter_by(
    ...
    social.updated_at = datetime.now(timezone.utc)
    db.session.commit()

    # --------------------------------------------------------
    # CONTINUE → AUTO ATTACH BUSINESS + AD ACCOUNT
    # --------------------------------------------------------
...
import json
from datetime import datetime, timezone
from models import db, SocialAccount, AdAccount
...
def fetch_business_assets(access_token, workspace_id, user_id):
    ...
    meta_log("CLIENT BUSINESS RESPONSE", biz_res)

    business_id = biz_res.get("client_business_id")
    if not business_id:
        return jsonify({
            "success": False,
    ...
    fetch("owned_ad_accounts")
    fetch("client_ad_accounts")

    if not ad_accounts:
        return jsonify({
            "success": False,
            "error": "no_ad_accounts_found"
        }), 404

    # --------------------------------------------------------
    # AUTO-SELECT FIRST AD ACCOUNT
    # --------------------------------------------------------
    selected = ad_accounts[0]
    ...
    profile = {
        "ad_account": {
            ...
            "business_name": None,
            "auto_attached_at": datetime.now(timezone.utc).isoformat(),
        }
    }

    social.ad_account_id = ad_account_id
    social.profile =profile
    social.updated_at = datetime.now(timezone.utc)

    # --------------------------------------------------------
    # UPSERT AD ACCOUNT ✅ FIXED (GLOBAL LOOKUP)
    # --------------------------------------------------------
...
def meta_save():
    ...
    # --------------------------------------------------
    # BUILD PROFILE (JSONB OBJECT — NO STRING)
    # --------------------------------------------------
    profile_dict = {
        "ad_account": {
    ...
    if not social:
        return jsonify({
            "success": False,
            "error": "facebook_social_account_not_found"
        }), 404

    # --------------------------------------------------
    # UPDATE FIELDS
    # --------------------------------------------------
    social.profile = profile_dict              # ✅ JSONB object
    social.ad_account_id = ad_account_id

    # ✅ UPDATE SCOPES (comma-separated string)
    if "scopes" in data and isinstance(data["scopes"], str):
        social.scopes = data["scopes"]
    ...
    return jsonify({
        ...
        "user_id": user_id,
        "ad_account_id": ad_account_id,
        "profile": profile_dict,     # object, not string
...
def test_system_token():
    """
    Debug endpoint to verify SYSTEM USER TOKEN works
    """
    ...
    meta_log("SYSTEM TOKEN TEST RESPONSE", res)
    return jsonify(res)


@app.route("/api/meta/deauthorize", methods=["POST"])
def meta_deauthorize():
    meta_log("DEAUTHORIZE CALLED")
    return jsonify({"success": True})

@app.route("/api/meta/data-deletion", methods=["POST"])
def meta_data_deletion():
    meta_log("DATA DELETION REQUEST")
    return jsonify({
        "url": f"{os.getenv('FRONTEND_BASE_URL')}/data-deletion-status"
    })
...
def fetch_ad_account_details(ad_account_id: str, access_token: str, api_version="v18.0"):
...
@app.route("/api/facebook/ad-account/details", methods=["POST"])
def get_ad_account_details():
    ...
    if not workspace_id or not user_id:
        return jsonify({"ok": False, "error": "workspace_id and user_id required"}), 400

    fb_data = _get_fb_account_for_workspace_user(str(workspace_id), str(user_id))
    if not fb_data:
        return jsonify({"ok": False, "error": "meta_account_not_connected"}), 400

    access_token = fb_data.get("access_token")
    ad_account_id = fb_data.get("ad_account_id")

    if not access_token or not ad_account_id:
        return jsonify({"ok": False, "error": "missing_ad_account"}), 400

    details = fetch_ad_account_details(ad_account_id, access_token)

    if details.get("error"):
        return jsonify({
            "ok": False,
            "error": "ad_account_fetch_failed",
            "details": details
        }), 400
...
@app.route("/api/social/management/full", methods=["GET"])
def api_social_management_full():
    """
    FULL Meta discovery: businesses, ad accounts, billing, pages, IG, DB state
    """

    workspace_id = request.args.get("workspace_id")
    if not workspace_id:
        return jsonify({"success": False, "error": "workspace_id_required"}), 400
    ...
    db_ad_accounts = {s.ad_account_id for s in social_accounts if s.ad_account_id}
    access_token = social_accounts[0].access_token if social_accounts else None

    if not access_token:
        return jsonify({"success": False, "error": "no_meta_token_found"}), 400

    # --------------------------------------------------
    # FACEBOOK USER
    # --------------------------------------------------
    _, _, fb_user = _graph_get(
        "me",
        access_token,
    ...
    # BUSINESS MANAGERS
    # --------------------------------------------------
    _, _, bm_res = _graph_get(
        "me/businesses",
        access_token,
        params={"fields": "id,name,verification_status"}
    )
    businesses = bm_res.get("data", [])

    all_ad_accounts = []
    all_billing = []

    # --------------------------------------------------
    # PER BUSINESS → AD ACCOUNTS + BILLING
    # --------------------------------------------------
        ...
        for edge in ["owned_ad_accounts", "client_ad_accounts"]:
            ...
            for a in res.get("data", []):
                ...
                ad_info = {
                    ...
                    "business_id": bm_id,
                    "source": edge,
                    "linked_in_db": ad_id in db_ad_accounts,
                }

                all_ad_accounts.append(ad_info)

                # ------------------------------
                # BILLING / FUNDING SOURCES
                # ------------------------------
                ok, _, bill = _graph_get(
                    f"act_{ad_id}/funding_source_details",
                    access_token,
                    params={"fields": "id,type,display_string"}
                )

                for fs in bill.get("data", []):
                    all_billing.append({
                        "ad_account_id": ad_id,
                        "funding_source_id": fs.get("id"),
                        "type": fs.get("type"),
                        "display": fs.get("display_string"),
                    })

    # --------------------------------------------------
    # FACEBOOK PAGES
    # --------------------------------------------------
    _, _, pages_res = _graph_get(
        "me/accounts",
        access_token,
        params={"fields": "id,name,category,tasks,instagram_business_account"}
    )

    pages = []
    instagram_accounts = []

    for p in pages_res.get("data", []):
        pages.append({
            "id": p["id"],
            "name": p["name"],
            "category": p.get("category"),
    ...
    return jsonify({
        ...
        "database_state": {
            "linked_social_accounts": [s.serialize() for s in social_accounts],
        },
...
if not EXTERNAL_BASE_URL:
    EXTERNAL_BASE_URL = "https://rlcslwgm-5000.inc1.devtunnels.ms"
...
def _generate_image_from_prompt(
    ...
    """
    Generates images using the GenAI client.
    Handles aspect_ratio correctly for Imagen/Gemini models.
    """
    if not GENAI_CLIENT:
    ...
    # Validate Aspect Ratio (Gemini/Imagen usually supports specific ratios)
    valid_ratios = ["1:1", "16:9", "9:16", "4:3", "3:4"]
    if aspect_ratio and aspect_ratio not in valid_ratios:
        # Fallback for close matches or defaults
        print(f"[Warn] Unsupported aspect ratio {aspect_ratio}, defaulting to 1:1")
        aspect_ratio = "1:1"
    ...
    # Apply Image Config
    if aspect_ratio:
        # Try-except block to handle different SDK versions of ImageConfig
        try:
            if IMAGE_CONFIG_IS_CLASS:
                config_args["image_config"] = ImageConfig(aspect_ratio=aspect_ratio)
            else:
                config_args["image_config"] = {"aspect_ratio": aspect_ratio}
        except Exception as e:
            print(f"[GenAI] Error setting image_config: {e}")

    # Apply Safety Settings (Permissive for creative work)
    ...
    try:
        ...
        print(f"[_generate_image] Model: {model_id}, Ratio: {aspect_ratio}")
        ...
        print(f"[_generate_image] Generation failed: {e}")
        # Return empty object or raise depending on error handling strategy
        raise e
...
def generate_image_candidates(
    ...
    """
    resp = _generate_image_from_prompt(
        prompt_text, 
        model_id=model_id, 
        candidate_count=n, 
...
# --- Image Proxy Endpoint (bypass CORS for CDN images) ---

@app.route("/api/v1/image-proxy", methods=["GET", "OPTIONS"])
def image_proxy_endpoint():
    """
    Proxy images from DigitalOcean Spaces CDN to bypass CORS restrictions.
    
    Usage: GET /api/v1/image-proxy?url=<encoded_cdn_url>
    ...
    try:
        ...
        allowed_domains = [
            "sociovia.blr1.cdn.digitaloceanspaces.com",
            "sociovia.blr1.digitaloceanspaces.com",
        ]
        from urllib.parse import urlparse
        parsed = urlparse(image_url)
        if parsed.netloc not in allowed_domains:
            return jsonify({"error": "Only CDN URLs can be proxied"}), 403
        
        # Fetch the image
        resp = requests.get(image_url, timeout=30, stream=True)
        if resp.status_code != 200:
            return jsonify({"error": f"Failed to fetch image: {resp.status_code}"}), resp.status_code
        ...
        proxy_response.headers["Cache-Control"] = "public, max-age=86400"  # Cache for 24 hours
        
        return proxy_response
        
    except Exception as e:
        current_app.logger.exception("image_proxy_endpoint error")
...
def save_image_endpoint():
    ...
    Supports 15-day expiration by default.
    ...
    Accepts application/json:
    {
        ...
        "user_id": "number",
        "workspace_id": "string",
        "expires_at": "2025-12-21T10:30:00.000Z",  // Optional: ISO datetime string
        "expiry_days": 15,  // Optional: Number of days (default 15)
        "is_permanent": false  // Optional: If true, never expires
    }
    
    Returns: { success: true, id: "<saved-id>", url: "<cdn url>", expires_at: "...", is_permanent: false }
    """
    try:
        ...
        if not copied and (url.startswith("http://") or url.startswith("https://")):
            ...
            if download_result:
                img_data, mime = download_result
                try:
                    s3.put_object(Bucket=SPACE_NAME, Key=saved_key, Body=img_data, ContentType=mime, ACL='public-read')
                    copied = True
        ...
        print(f"[save-image] Received user_id: {user_id}, workspace_id: {workspace_id}, expires_at: {expires_at}, is_permanent: {is_permanent}")
        try:
            if user_id and workspace_id:
                # Save to Creative table (backward compatibility)
                creative = Creative(
                    id=saved_id,
                    user_id=user_id,
                    workspace_id=workspace_id,
                ...
                # Save to SavedImage table with expiration
                ...
                db.session.commit()
                print(f"[save-image] Stored saved image {saved_id} in DB for user {user_id} / workspace {workspace_id} (expires: {expires_at})")
        except Exception as e:
...
@app.route("/api/v1/images/mark-permanent", methods=["POST"])
def mark_image_permanent():
    """
    Mark an image as permanent (never expires).
    
    Request body:
    {
        "image_id": "string",
        "url": "string",  // optional, for fallback lookup
        "user_id": "number",
        "workspace_id": "string"
    }
    ...
    Response:
    {
        "success": true,
        "id": "image_id",
        "message": "Image saved permanently"
    }
    """
    try:
        data = request.get_json(force=False, silent=True) or {}
        ...
        if not image_id and not url:
            return jsonify({"success": False, "error": "image_id or url required"}), 400
        ...
        saved_image = None
        if image_id:
            saved_image = SavedImage.query.filter_by(id=image_id, user_id=str(user_id)).first()
        
        if not saved_image and url:
            saved_image = SavedImage.query.filter_by(url=url, user_id=str(user_id)).first()
        
        if not saved_image:
            # Image not found in SavedImage table, try to create one from existing data
            if url and workspace_id:
                # Create a new permanent saved image entry
                saved_id = image_id or uuid.uuid4().hex
                now = datetime.now(timezone.utc)
                saved_image = SavedImage(
                    id=saved_id,
                    user_id=str(user_id),
                ...
                db.session.add(saved_image)
                db.session.commit()
                print(f"[mark-permanent] Created new permanent saved image {saved_id} for user {user_id}")
                return jsonify({
                    "success": True,
                    "id": saved_id,
                    "message": "Image saved permanently"
        ...
        # Update the image to be permanent
        saved_image.is_permanent = True
        saved_image.expires_at = None
        saved_image.expiry_notified = False
        db.session.commit()
        
        print(f"[mark-permanent] Marked image {saved_image.id} as permanent for user {user_id}")
        ...
        print(f"[mark-permanent] exception: {e}")
        db.session.rollback()
        return jsonify({"success": False, "error": "server_error", "details": str(e)}), 500


@app.route("/api/v1/images/expiring", methods=["GET"])
def get_expiring_images():
    """
    Get images expiring within N days for a user.
    
    Query params:
    - user_id (required): User ID
    - workspace_id (optional): Filter by workspace
    - days (optional, default: 3): Get images expiring within this many days
        ...
        "images": [
            {
                "id": "img_123",
                "url": "https://...",
                "expires_at": "2025-12-21T10:30:00.000Z",
                "days_remaining": 2,
                "workspace_id": "ws_456"
            }
        ],
    ...
    try:
        ...
        # Build query
        query = SavedImage.query.filter(
            SavedImage.user_id == str(user_id),
            SavedImage.is_permanent == False,
            SavedImage.expires_at != None,
            SavedImage.expires_at <= expiry_threshold,
            SavedImage.expires_at > now
        )
        
        if workspace_id:
            query = query.filter(SavedImage.workspace_id == str(workspace_id))
        
        expiring_images = query.order_by(SavedImage.expires_at.asc()).all()
        
        images_data = []
        for img in expiring_images:
            delta = img.expires_at - now
            days_remaining = max(0, delta.days)
            images_data.append({
                "id": img.id,
                "url": img.url,
        ...
        print(f"[expiring] Found {len(images_data)} expiring images for user {user_id} (within {days} days)")
        ...
        print(f"[expiring] exception: {e}")
        return jsonify({"success": False, "error": "server_error", "details": str(e)}), 500


@app.route("/api/v1/images/expired", methods=["DELETE"])
def delete_expired_images():
    """
    Delete all expired images for a user.
    ...
    try:
        ...
        # Build query for expired images
        query = SavedImage.query.filter(
            SavedImage.user_id == str(user_id),
            SavedImage.is_permanent == False,
            SavedImage.expires_at != None,
        ...
        for img in expired_images:
            ...
            if img.saved_key:
                try:
                    s3.delete_object(Bucket=SPACE_NAME, Key=img.saved_key)
                    print(f"[expired] Deleted {img.saved_key} from storage")
                except Exception as e:
                    print(f"[expired] Failed to delete {img.saved_key} from storage: {e}")
            
            deleted_images.append({"id": img.id, "url": img.url})
            
            # Delete from database
            db.session.delete(img)
            
            # Also delete from Creative table if exists
            try:
                creative = Creative.query.filter_by(id=img.id).first()
                if creative:
        ...
        print(f"[expired] Deleted {len(deleted_images)} expired images for user {user_id}")
...
def _generate_image_from_prompt_v2(
    prompt_text: str,
    model_id: str = MODEL_ID,
    candidate_count: int = 1,
    guidance_scale: float | None = None,
    ...
    """
    Alternate implementation - uses GENAI_CLIENT.models.generate_content with contents=[prompt_text].
    Supports aspect_ratio, Google Search, and ThinkingConfig.
    """
    if not GENAI_CLIENT:
        raise RuntimeError("GenAI client not initialized")
    ...
    # Build contents using types.Content for proper structure
    contents = [
        types.Content(
            role="user",
            parts=[
    ...
    if use_google_search:
        try:
            ...
            print(f"[_generate_image_from_prompt_v2] GoogleSearch tool init failed: {e}", flush=True)

    # Build safety settings (turn off for creative generation)
    safety_settings = [
        types.SafetySetting(category="HARM_CATEGORY_HATE_SPEECH", threshold="OFF"),
        types.SafetySetting(category="HARM_CATEGORY_DANGEROUS_CONTENT", threshold="OFF"),
        types.SafetySetting(category="HARM_CATEGORY_SEXUALLY_EXPLICIT", threshold="OFF"),
    ...
    try:
        ...
        # Add aspect ratio via ImageConfig if provided
        if aspect_ratio:
            try:
                cfg_kwargs["image_config"] = ImageConfig(aspect_ratio=aspect_ratio)
                print(f"[_generate_image_from_prompt_v2] Using aspect_ratio: {aspect_ratio}", flush=True)
            except Exception as e:
                print(f"[_generate_image_from_prompt_v2] ImageConfig failed: {e}", flush=True)
        
        # Add tools if any
        if tools:
            cfg_kwargs["tools"] = tools
        
        # Add thinking config for higher quality
        if thinking_level:
            try:
                cfg_kwargs["thinking_config"] = types.ThinkingConfig(thinking_level=thinking_level)
                print(f"[_generate_image_from_prompt_v2] Using thinking_level: {thinking_level}", flush=True)
            except Exception as e:
                print(f"[_generate_image_from_prompt_v2] ThinkingConfig failed: {e}", flush=True)
        ...
        print(f"[_generate_image_from_prompt_v2] Config build failed: {e}, using minimal config", flush=True)
        cfg = {
            "response_modalities": ["TEXT", "IMAGE"],
            "candidate_count": 1,
            "max_output_tokens": max_output_tokens,
    ...
    print(f"[_generate_image_from_prompt_v2] Using model={model_id}, aspect_ratio={aspect_ratio}, thinking={thinking_level}", flush=True)
    ...
    try:
        ...
        print(f"[_generate_image_from_prompt_v2] multi-candidate call failed: {e}. Falling back to repeated single-candidate calls.", flush=True)
    ...
    for i in range(max(1, candidate_count)):
        try:
            ...
            if aspect_ratio:
                try:
                    fallback_cfg_kwargs["image_config"] = ImageConfig(aspect_ratio=aspect_ratio)
                except Exception:
                    pass
            if guidance_scale is not None:
                fallback_cfg_kwargs["guidance_scale"] = guidance_scale
            single_cfg = types.GenerateContentConfig(**fallback_cfg_kwargs)

            r = GENAI_CLIENT.models.generate_content(model=model_id, contents=contents, config=single_cfg)
            responses.append(r)
        except Exception as e2:
            print(f"[_generate_image_from_prompt_v2] candidate call {i} failed: {e2}", flush=True)
            # continue to next candidate
    return responses


def generate_image_candidates_v2(
    prompt_text: str, 
    model_id: str = MODEL_ID, 
    n: int = 3, 
    guidance_scale: float | None = None,
    ...
    """
    resp = _generate_image_from_prompt_v2(
        prompt_text, 
        model_id=model_id, 
        candidate_count=n, 
        guidance_scale=guidance_scale,
...
def generate():
    ...
    # Get aspect_ratio from payload (supports multiple field names)
    requested_aspect_ratio = (
        data.get("aspect_ratio") or 
        data.get("aspectRatio") or 
        data.get("ratio") or
        (data.get("options") or {}).get("aspect_ratio") or
        (data.get("options") or {}).get("aspectRatio")
    )
    # Get platform and map to aspect ratio if not explicitly provided
    platform = data.get("platform") or data.get("target_platform")
    if not requested_aspect_ratio and platform:
        requested_aspect_ratio = PLATFORM_ASPECT_MAP.get(platform)
    
    # Get optional generation settings
    use_google_search = data.get("use_google_search", False)
    thinking_level = data.get("thinking_level")  # e.g., "HIGH", "MEDIUM", "LOW"
    
    dbg(f"requested_aspect_ratio: {requested_aspect_ratio}, platform: {platform}, thinking: {thinking_level}")
...
"""
    ...
    for idx, theme in enumerate(themes):
        ...
        try:
            ...
            # Determine aspect ratio: use requested_aspect_ratio if provided, else from theme, else default to 1:1
            theme_aspect = theme.get('aspect_ratio', '1:1')
            final_aspect_ratio = requested_aspect_ratio or theme_aspect
            dbg(f"[Theme {idx}] Using aspect_ratio: {final_aspect_ratio}")

            # Generate exactly 1 candidate (n=1) with aspect_ratio, google search, and thinking config
            candidate_responses = generate_image_candidates(
                full_prompt, 
    ...
    # Save conversation
    try:
            # Extract user_id and workspace_id from request if not already defined
        if not user_id:
            user_id = request.headers.get("X-User-Id")
        if not workspace_id:
...
@app.route("/api/v1/edit-image", methods=["POST", "OPTIONS"])
def edit_image_endpoint():
    ...
    if request.method == "OPTIONS":
        ...
        resp.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-Requested-With, Accept, X-User-Id, X-Workspace-Id, X-User-Email"
        resp.headers["Access-Control-Allow-Credentials"] = "true"
        return resp
    if not GENAI_CLIENT:
        return jsonify({"success": False, "error": "genai_client_not_initialized"}), 500
    ...
    try:
        ...
        # Save edited images to Creative model
        print(f"[edit-image2] Received user_id: {user_id}, workspace_id: {workspace_id}")
        try:
            if user_id and workspace_id and saved_files:
                for fn, url in zip(saved_files, urls):
                    ...
                    db.session.add(creative)
                db.session.commit()
                print(f"[edit-image2] Stored {len(saved_files)} edited images in DB for user {user_id} / workspace {workspace_id}")
        except Exception as e:
            print(f"[edit-image2] DB commit failed: {str(e)}")
            db.session.rollback()
...
def extract_workspace_context_from_prompt(prompt: str) -> dict:
    """
    Extract workspace details from prompt containing embedded JSON.
    Returns dict with business context keys for use in image generation variations.
    """
    if not prompt or "Workspace Details (JSON):" not in prompt:
        return {}
    try:
        match = re.search(r'Workspace Details \(JSON\):\s*(\{[\s\S]*?\})\s*---', prompt)
        if match:
            ws = json.loads(match.group(1))
            return {
                "business_name": ws.get("business_name", ""),
                "industry": ws.get("industry", ""),
                "usp": ws.get("usp", ""),
                "description": ws.get("description", ""),
                "audience": ws.get("audience_description", ""),
            }
    except Exception as e:
        # Log but don't fail - graceful degradation
        try:
            current_app.logger.debug(f"Could not extract workspace context: {e}")
        except:
            pass
    return {}
...
def generate_from_image_endpoint():
    ...
    try:
        ...
        if request.content_type and request.content_type.startswith("multipart/form-data"):
            ...
            edit_only = (request.form.get("edit_only") or "").lower() in ("1", "true", "yes")
            file_uri = request.form.get("file_uri") or request.form.get("image_url")
            user_id = request.form.get("user_id") or request.headers.get("X-User-Id")
            workspace_id = request.form.get("workspace_id") or request.headers.get("X-Workspace-Id")
        ...
        # Try to extract workspace_id from embedded JSON in prompt if not provided
        if not workspace_id and "Workspace Details (JSON):" in prompt:
            try:
                import re
                json_match = re.search(r'\{[^{}]*"id"\s*:\s*(\d+)[^{}]*\}', prompt)
                if json_match:
                    workspace_id = json_match.group(1)
                    current_app.logger.info(f"Extracted workspace_id={workspace_id} from prompt")
            except Exception as e:
                current_app.logger.debug(f"Could not extract workspace_id from prompt: {e}")

        # Try to extract user_id from workspace JSON if not provided
        if not user_id and "Workspace Details (JSON):" in prompt:
            try:
                import re
                user_match = re.search(r'"user_id"\s*:\s*(\d+)', prompt)
                if user_match:
                    user_id = user_match.group(1)
                    current_app.logger.info(f"Extracted user_id={user_id} from prompt")
            except Exception as e:
                current_app.logger.debug(f"Could not extract user_id from prompt: {e}")
        ...
        # Extract workspace context BEFORE sanitization strips it away
        ws_ctx = extract_workspace_context_from_prompt(prompt)
        business = ws_ctx.get("business_name", "")
        industry = ws_ctx.get("industry", "")
        usp = ws_ctx.get("usp", "")
        
        # Build context string for variations
        ctx_parts = []
        if business:
            ctx_parts.append(f"for {business}")
        if industry:
            ctx_parts.append(f"in {industry}")
        if usp:
            ctx_parts.append(f"showcasing: {usp}")
        context_str = " ".join(ctx_parts) if ctx_parts else ""
        
        # Define the function to call model per variation
        def call_model_for_variation(variation_label: str):
            parts = []
            
            # Attach Image if present (Img2Img)
            if file_bytes:
                parts.append(Part.from_bytes(data=file_bytes, mime_type=mime_type or "image/png"))
            elif file_uri:
                parts.append(Part.from_uri(file_uri=file_uri))

            # Add Text Instruction
            instruction = f"{sanitized_prompt}\n\n{variation_label}"
            parts.append(Part.from_text(text=instruction))
            ...
            cfg = GenerateContentConfig(
                ...
                top_p=0.95,
                max_output_tokens=8192,
                response_modalities=["IMAGE"], # Force Image
                candidate_count=1,
                # KEY FIX: Pass aspect ratio here
                image_config=ImageConfig(aspect_ratio=aspect_ratio) if IMAGE_CONFIG_IS_CLASS else {"aspect_ratio": aspect_ratio}
            )
            ...
            try:
                ...
                # Extract Images
                saved = save_images_from_response(resp, prefix=f"var_{uuid.uuid4().hex[:6]}")
                
                # Format Result
                if saved:
                    urls = [f"{SPACE_CDN}/outputs/{fn}" if SPACE_CDN else f"/outputs/{fn}" for fn in saved]
                    return {
                        "variation": variation_label, 
                        "success": True, 
                        "files": saved, 
                    ...
                    return {"variation": variation_label, "success": False, "error": "no_image_generated"}

            except Exception as e:
                current_app.logger.error(f"Variation failed: {e}")
                return {"variation": variation_label, "success": False, "error": str(e)}

        # ------------------------
        # Execution
        # ------------------------
        
        # Context-aware variation prompts for better consistency
        if context_str:
            variations = [
                f"Create a professional marketing visual {context_str}. Use clean, modern design with strong visual hierarchy. Aspect ratio {aspect_ratio}.",
                f"Generate an engaging social media graphic {context_str}. Make it attention-grabbing and shareable. Aspect ratio {aspect_ratio}.",
                f"Design a brand-focused visual {context_str}. Emphasize trust, credibility, and professionalism. Aspect ratio {aspect_ratio}."
            ]
        else:
            # Fallback to improved generic prompts
            variations = [
                f"Create a professional marketing visual with clean, modern design. Aspect ratio {aspect_ratio}.",
                f"Generate an engaging social media graphic that is attention-grabbing. Aspect ratio {aspect_ratio}.",
                f"Design a creative, shareable visual for social media. Aspect ratio {aspect_ratio}."
            ]

        results = []
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            futures = {ex.submit(call_model_for_variation, v): v for v in variations}
            for fut in as_completed(futures):
        ...
        # DB Persistence - Save as Conversation
        conv_id = None
        if user_id and workspace_id and saved_files:
            try:
                conv_id = uuid.uuid4().hex
                ...
                # Clean prompt for display (strip workspace details)
                display_prompt = prompt
                if "---" in prompt and "Workspace Details" in prompt:
                    display_prompt = prompt.split("---")[0].strip()
                display_prompt = display_prompt[:200] if display_prompt else "Image generation"
                
                conversation = Conversation(
                    id=conv_id,
                    user_id=str(user_id),
                    workspace_id=str(workspace_id),
                    session_id=session_id,  # For grouping prompts in same chat session
                    prompt=display_prompt,
                    response=json.dumps(response_data)
                )
                db.session.add(conversation)
                db.session.commit()
                current_app.logger.info(f"Saved conversation {conv_id} for user {user_id} workspace {workspace_id} session {session_id}")
            except Exception as e:
                db.session.rollback()
                current_app.logger.error(f"DB Save failed: {e}")
        ...
        return jsonify({
            ...
            "session_id": session_id  # Return session_id for frontend grouping
        }), 200

    except Exception as e:
        current_app.logger.exception("Endpoint failed")
        return jsonify({"success": False, "error": str(e)}), 500


# Removed duplicate generate-from-images endpoint code
...
# -------------------------
FB_GRAPH_API_VERSION = os.getenv("FB_GRAPH_API_VERSION", "v22.0")


JSON_EXTRACT_REGEX = re.compile(r"(\{[\s\S]*\}|\[[\s\S]*\])", re.MULTILINE)


def _safe_extract_text_from_resp1(
    resp: Any,
    *,
    file_bytes_list: Optional[List[bytes]] = None,
    ...
    # SAFE DEFAULTS
    # -------------------------
    file_bytes_list = file_bytes_list or []
    ...
    print(
        f"[generate-from-images] initial file_bytes_list count: {len(file_bytes_list)} "
        f"file_uris count: {len(file_uris)}",
        flush=True,
    )
    ...
    if use_themes:
        try:
            first = None
            if file_bytes_list:
                first = Part.from_bytes(
                    data=file_bytes_list[0],
        ...
        for idx, theme in enumerate(themes):
            ...
            if not visual_prompt:
                results.append(
                    {"theme_index": idx, "error": "missing_visual_prompt"}
                )
                continue
            ...
            final_prompt = (
                f"{visual_prompt}\n\n"
                f"User prompt (priority): {escape_for_inline(prompt)}\n"
                f"Attached_prompt: {escape_for_inline(theme.get('attached_prompt', ''))}"
            )

            try:
                img_resp = _generate_image_with_input_images(
                    final_prompt,
                    parts,
                    model_id=MODEL_ID,
                    aspect_ratio=aspect_ratio,
            ...
            saved = save_images_from_response(
                img_resp,
                prefix=f"multi_theme{idx}",
            )
    ...
    for i, b in enumerate(file_bytes_list):
        parts.append(
            Part.from_bytes(
                data=b,
                mime_type=mime_types[i] if i < len(mime_types) else "image/png",
            )
        )
    for uri in file_uris:
        parts.append(Part.from_uri(file_uri=uri))
    ...
    final_prompt = (
        f"User prompt (priority): {escape_for_inline(prompt)}\n"
        f"Blend supplied reference images correctly."
    )
...
def detect_content_type_by_bytes(image_bytes):
    try:
        with Image.open(io.BytesIO(image_bytes)) as img:
            ...
            return mapping.get(fmt, (None, None))
    except Exception:
        return None, None
...
def upload_image_bytes(account_id, image_bytes, content_type=None):
    try:
        ...
        if not content_type or not content_type.startswith("image/"):
            detected_type, filename = detect_content_type_by_bytes(image_bytes)
...
# ============================================================================
# VIDEO UPLOAD SUPPORT FOR META/FACEBOOK ADS
# ============================================================================
...
def _detect_video_content_type(video_bytes: bytes) -> Optional[str]:
    """Detect video content type from magic bytes."""
    ...
    if video_bytes[4:8] == b'ftyp':
        ...
        if ftyp in (b'isom', b'iso2', b'mp41', b'mp42', b'avc1', b'M4V ', b'dash'):
            return "video/mp4"
        if ftyp in (b'qt  ', b'moov'):
            return "video/quicktime"
        # Default to mp4 for other ftyp variants
        return "video/mp4"
    ...
    # AVI detection (RIFF....AVI)
    if video_bytes[:4] == b'RIFF' and video_bytes[8:12] == b'AVI ':
        return "video/x-msvideo"
    
    # MKV detection (EBML with matroska doctype)
    if video_bytes[:4] == b'\x1a\x45\xdf\xa3':
        # Could be WebM or MKV - check for matroska marker
        if b'matroska' in video_bytes[:100]:
...
def _get_video_extension(content_type: str) -> str:
    """Get file extension for video content type."""
    return SUPPORTED_VIDEO_FORMATS.get(content_type, ".mp4")


def upload_video_to_digitalocean(video_bytes: bytes, filename: str, content_type: str = "video/mp4") -> Optional[str]:
    """
    Upload video to DigitalOcean Spaces and return public CDN URL.
    
    Uses existing S3 client configuration from environment:
    - SPACE_NAME / DO_SPACES_BUCKET: Bucket name
    - SPACE_REGION / DO_SPACES_REGION: Region (e.g., 'blr1', 'nyc3')
    - ACCESS_KEY / DO_SPACES_KEY: Access key
    - SECRET_KEY / DO_SPACES_SECRET: Secret key
    
    Returns the public CDN URL or None on failure.
    """
    if not video_bytes:
        logger.warning("upload_video_to_digitalocean: empty video bytes")
        return None
    
    if len(video_bytes) > MAX_VIDEO_SIZE_BYTES:
        logger.error("upload_video_to_digitalocean: video exceeds max size (%d > %d)", len(video_bytes), MAX_VIDEO_SIZE_BYTES)
        return None
    
    # Get S3 client from globals (already configured at startup)
    s3_client = globals().get("s3")
    bucket_name = globals().get("SPACE_NAME") or os.environ.get("SPACE_NAME") or os.environ.get("DO_SPACES_BUCKET")
    space_region = globals().get("SPACE_REGION") or os.environ.get("SPACE_REGION") or os.environ.get("DO_SPACES_REGION") or "blr1"
    space_cdn = globals().get("SPACE_CDN")
    
    if not s3_client:
        logger.error("upload_video_to_digitalocean: S3 client not configured")
        return None
    
    if not bucket_name:
        logger.error("upload_video_to_digitalocean: SPACE_NAME/DO_SPACES_BUCKET not configured")
        return None
    
    # Generate unique filename with timestamp
    timestamp = int(time.time() * 1000)
    unique_id = uuid.uuid4().hex[:8]
    ext = _get_video_extension(content_type)
    
    # Clean filename
    if filename:
        # Remove extension from provided filename and add proper one
        base_name = os.path.splitext(filename)[0]
        base_name = re.sub(r'[^a-zA-Z0-9_-]', '_', base_name)[:50]  # Sanitize
    else:
        base_name = "video"
    
    final_filename = f"{base_name}_{timestamp}_{unique_id}{ext}"
    key = f"videos/{final_filename}"
    ...
    try:
        ...
        # Build public URL - prefer CDN if configured
        if space_cdn:
            public_url = f"{space_cdn}/{key}"
        else:
            public_url = f"https://{bucket_name}.{space_region}.digitaloceanspaces.com/{key}"
        
        logger.info("upload_video_to_digitalocean: successfully uploaded video to %s", public_url)
        return public_url
        
    except Exception as e:
        logger.exception("upload_video_to_digitalocean: upload failed: %s", e)
        return None


def upload_video_bytes_to_fb(ad_account_id: str, video_bytes: bytes, content_type: str = None, filename: str = None) -> Optional[str]:
    """
    Upload video bytes to Facebook Ads API using the advideos endpoint.
    Returns the video_id on success, None on failure.
    
    Facebook video upload uses multipart form with 'source' field.
    """
    if not video_bytes:
        logger.warning("upload_video_bytes_to_fb: empty video bytes")
        return None
    
    if len(video_bytes) > MAX_VIDEO_SIZE_BYTES:
        logger.error("upload_video_bytes_to_fb: video exceeds max size (%d > %d)", len(video_bytes), MAX_VIDEO_SIZE_BYTES)
        return None
    
    # Detect content type if not provided
    if not content_type:
        content_type = _detect_video_content_type(video_bytes)
        if not content_type:
            logger.warning("upload_video_bytes_to_fb: could not detect video type, defaulting to video/mp4")
            content_type = "video/mp4"
    
    # Validate content type
    if content_type not in SUPPORTED_VIDEO_FORMATS:
        logger.warning("upload_video_bytes_to_fb: unsupported content type %s", content_type)
        # Try to proceed anyway, Facebook might accept it
    ...
    try:
        acct = _acct_prefix(ad_account_id)
        
        # Use multipart upload with 'source' field
        files = {
            "source": (filename, io.BytesIO(video_bytes), content_type)
        }
        
        resp = fb_post(f"{acct}/advideos", files=files, timeout=120)  # Longer timeout for video
        
        if isinstance(resp, dict):
            # Success response contains 'id' for the video
            video_id = resp.get("id") or resp.get("video_id")
            if video_id:
                logger.info("upload_video_bytes_to_fb: successfully uploaded video, id=%s", video_id)
                return str(video_id)
            
            # Check for error
            if resp.get("error"):
                logger.error("upload_video_bytes_to_fb: API error: %s", resp.get("error"))
                return None
        
        logger.warning("upload_video_bytes_to_fb: unexpected response: %s", resp)
        return None
        
    except Exception as e:
        logger.exception("upload_video_bytes_to_fb exception: %s", e)
        return None


def upload_video_by_url_to_fb(ad_account_id: str, video_url: str) -> Optional[str]:
    """
    Upload a video to Facebook by URL.
    ...
    try:
        ...
        # Try direct URL upload first (Facebook fetches from URL)
        logger.info("upload_video_by_url_to_fb: attempting direct URL upload from %s", video_url)
        resp = fb_post(f"{acct}/advideos", data={"file_url": video_url}, timeout=180)  # Longer timeout
        
        if isinstance(resp, dict):
            video_id = resp.get("id") or resp.get("video_id")
            if video_id:
                logger.info("upload_video_by_url_to_fb: successfully uploaded video from URL, id=%s", video_id)
                return str(video_id)
            
            # If URL upload failed, log the error
            if resp.get("error"):
                logger.warning("upload_video_by_url_to_fb: direct URL upload failed: %s", resp.get("error"))
                
                # Fallback: download and upload directly
                logger.info("upload_video_by_url_to_fb: trying download fallback...")
                try:
                    ...
                    if video_bytes:
                        return upload_video_bytes_to_fb(ad_account_id, video_bytes, content_type)
                except Exception as dl_err:
                    logger.exception("upload_video_by_url_to_fb: download fallback failed: %s", dl_err)
        
        return None
        
    except Exception as e:
        logger.exception("upload_video_by_url_to_fb exception: %s", e)
        return None


def resolve_video_id(body: dict, video_file=None) -> tuple:
    """
    Resolve video from request - uploads to DigitalOcean Spaces first, then to Facebook.
    
    Flow:
    1. Decode video from base64/file/URL
    2. Upload to DigitalOcean Spaces to get CDN URL
    3. Upload to Facebook using that CDN URL
    
    Returns tuple: (video_id, cdn_url) or (None, None) if no video provided/upload failed.
    """
    creative_in = (body.get("creative") or {}) if isinstance(body, dict) else {}
    
    # Extract video fields from creative or body
    video_base64 = (creative_in.get("video_base64") or creative_in.get("videoBase64") or 
                    body.get("video_base64") or body.get("videoBase64"))
    video_url = (creative_in.get("video_url") or creative_in.get("videoUrl") or 
                 body.get("video_url") or body.get("videoUrl"))
    video_content_type = (creative_in.get("video_content_type") or creative_in.get("videoContentType") or
                          body.get("video_content_type") or body.get("videoContentType") or "video/mp4")
    video_filename = (creative_in.get("video_filename") or creative_in.get("videoFilename") or
                      body.get("video_filename") or body.get("videoFilename"))
    
    video_bytes = None
    content_type = video_content_type
    filename = video_filename
    
    # Priority 1: Base64 encoded video
    if video_base64:
        try:
            logger.info("resolve_video_id: processing base64 video")
            # Handle data URI format: data:video/mp4;base64,XXXX
            if video_base64.startswith("data:"):
                match = DATA_URI_RE.match(video_base64)
                if match:
                    content_type = match.group("mediatype") or "video/mp4"
                ...
                if not content_type:
                    content_type = _detect_video_content_type(video_bytes) or "video/mp4"
        except Exception as e:
            logger.exception("resolve_video_id: error decoding base64 video: %s", e)
    
    # Priority 2: File upload
    elif video_file and getattr(video_file, "filename", ""):
        try:
            logger.info("resolve_video_id: processing uploaded file")
            content_type = getattr(video_file, "mimetype", None) or content_type or "video/mp4"
            filename = getattr(video_file, "filename", None) or filename
            video_bytes = video_file.read()
            
            # Rewind file for potential downstream use
            try:
                video_file.seek(0)
            except Exception:
                pass
        except Exception as e:
            logger.exception("resolve_video_id: error reading uploaded file: %s", e)
    
    # Priority 3: Direct URL (skip DigitalOcean, use URL directly for Facebook)
    elif video_url and not video_url.startswith("data:"):
        try:
            logger.info("resolve_video_id: using direct video URL: %s", video_url)
            video_id = upload_video_by_url_to_fb(FB_AD_ACCOUNT_ID, video_url)
            if video_id:
                return (video_id, video_url)  # Return the original URL as cdn_url
            logger.warning("resolve_video_id: direct URL upload failed")
            return (None, None)
        except Exception as e:
            logger.exception("resolve_video_id: error uploading from URL: %s", e)
            return (None, None)
    
    # If we have video bytes, upload to DigitalOcean first, then to Facebook
    if video_bytes:
        logger.info("resolve_video_id: uploading %d bytes to DigitalOcean Spaces", len(video_bytes))
        
        # Generate filename if not provided
        if not filename:
            ext = _get_video_extension(content_type)
            filename = f"ad_video_{int(time.time())}{ext}"
        
        # Step 1: Upload to DigitalOcean Spaces
        cdn_url = upload_video_to_digitalocean(video_bytes, filename, content_type)
        
        if not cdn_url:
            logger.error("resolve_video_id: failed to upload to DigitalOcean Spaces")
            # Fallback: try direct upload to Facebook
            logger.info("resolve_video_id: attempting direct upload to Facebook as fallback")
            video_id = upload_video_bytes_to_fb(FB_AD_ACCOUNT_ID, video_bytes, content_type, filename)
            return (video_id, None)
        
        logger.info("resolve_video_id: video uploaded to CDN: %s", cdn_url)
        
        # Step 2: Upload to Facebook using the CDN URL
        video_id = upload_video_by_url_to_fb(FB_AD_ACCOUNT_ID, cdn_url)
        
        if video_id:
            logger.info("resolve_video_id: successfully uploaded to Facebook, video_id=%s", video_id)
            return (video_id, cdn_url)
        else:
            logger.warning("resolve_video_id: Facebook upload failed, but CDN URL is available")
            # Return CDN URL even if Facebook upload failed - can retry later
            return (None, cdn_url)
...
def _resolve_workspace_and_user(body: dict):
    """
...
def _get_fb_account_for_workspace_user(workspace_id: str, user_id: str):
    if not workspace_id or not user_id:
        return None

    try:
        acct = (
            current_app.db.session.query(SocialAccount)
            .filter(SocialAccount.provider == "facebook")
            .filter(cast(SocialAccount.workspace_id, String) == str(workspace_id))
            .filter(cast(SocialAccount.user_id, String) == str(user_id))
            .order_by(SocialAccount.updated_at.desc())
            .first()
        )
    except Exception:
        current_app.logger.exception("Failed fetching Facebook SocialAccount")
        return None

    if not acct or not acct.access_token or not acct.ad_account_id:
        return None
    ...
    return {
        ...
        "page_id": acct.provider_user_id,
    }
    
    
@app.route("/api/publish_v2", methods=["OPTIONS", "POST"])
def publish_v2():
    if request.method == "OPTIONS":
        return make_response("", 200)
    ...
    try:
        ...
        workspace_id = body.get("workspace_id") or session.get("workspace_id")
        user_id = body.get("user_id") or session.get("user_id")
        api_version = body.get("api_version", "v23.0")

        if not workspace_id or not user_id:
            return jsonify({"ok": False, "error": "workspace_id and user_id required"}), 400

        # -------------------------------------------------
        # 🔐 TOKEN + AD ACCOUNT (DB ONLY)
        # -------------------------------------------------
        fb = _get_fb_account_for_workspace_user(str(workspace_id), str(user_id))
        if not fb:
            return jsonify({"ok": False, "error": "no_facebook_social_account"}), 400

        access_token = fb.get("access_token")
        ad_account_id = fb.get("ad_account_id")

        if not access_token or not ad_account_id:
            return jsonify({"ok": False, "error": "missing_ad_account_or_token"}), 400
        ...
        # -------------------------------------------------
        # 📄 FETCH + AUTO SELECT PAGE
        # -------------------------------------------------
        pages = fetch_pages_for_user(access_token, api_version)
        if not pages:
            return jsonify({"ok": False, "error": "no_facebook_pages_found"}), 400
        ...
        # -------------------------------------------------
        now = datetime.utcnow()
        campaign_name = "SOC_" + (body.get("campaign_name") or f"Campaign {now.isoformat()}")
        adset_name = "SOC_" + (body.get("adset_name") or f"AdSet {now.isoformat()}")
        ad_name = "SOC_" + (body.get("ad_name") or f"Ad {now.isoformat()}")

        # -------------------------------------------------
        # 💰 Budget & schedule
        # -------------------------------------------------
        daily_budget = str(int(float(body.get("daily_budget", 100)) * 100))
        ...
        OBJECTIVE_MAP = {
            ...
            "AWARENESS": ("OUTCOME_AWARENESS", "IMPRESSIONS"),
            "SALES": ("OUTCOME_SALES", "CONVERSIONS"),
        }

        req_obj = str(body.get("objective", "TRAFFIC")).upper()
        objective, optimization_goal = OBJECTIVE_MAP.get(
            req_obj, ("OUTCOME_TRAFFIC", "LINK_CLICKS")
        )
        ...
        campaign_resp = requests.post(
            ...
            data={
                ...
                "objective": objective,
                "status": "PAUSED",
                "special_ad_categories": json.dumps(body.get("special_ad_categories", [])),
            },
            timeout=30
        ).json()

        if campaign_resp.get("error"):
            return jsonify({"ok": False, "stage": "campaign", "details": campaign_resp}), 500
        ...
        # 🎯 TARGETING
        # -------------------------------------------------
        raw_targeting = body.get("targeting") or {}
        if isinstance(raw_targeting, str):
            raw_targeting = json.loads(raw_targeting)

        geo = raw_targeting.get("geo_locations") or {}
        if geo.get("custom_locations"):
            normalized_geo = {
                "custom_locations": geo["custom_locations"],
                "location_types": ["home", "recent"],
            }
        else:
            normalized_geo = {"countries": ["IN"]}

        targeting = {
            **{k: v for k, v in raw_targeting.items() if k != "geo_locations"},
            "geo_locations": normalized_geo,
        }
        ...
        adset_resp = requests.post(
            f"{GRAPH}/act_{ad_account_id}/adsets",
            params={"access_token": access_token},
            data={
                "name": adset_name,
                "campaign_id": campaign_id,
        ...
        if adset_resp.get("error"):
            return jsonify({"ok": False, "stage": "adset", "details": adset_resp}), 500

        adset_id = adset_resp["id"]

        # -------------------------------------------------
        # 🖼 IMAGE — FINAL, META-SAFE PIPELINE
        # -------------------------------------------------
        image_hash = creative.get("image_hash") or body.get("image_hash")
        raw_base64 = (
            creative.get("image_base64")
        ...
        if not image_hash:
            ...
            raw_base64 = re.sub(r"\s+", "", raw_base64)
            ...
            if len(image_bytes) > 30 * 1024 * 1024:
                return jsonify({
                    "ok": False,
                    "error": "image_upload_failed",
                    "message": "Image exceeds 30MB Meta limit"
                }), 400
        ...
        if not image_hash:
            return jsonify({
                "ok": False,
                "error": "image_upload_failed",
                "message": "Meta rejected image upload"
            }), 400
        ...
        link_data = {
            "link": body.get("link"),
            "message": body.get("message") or body.get("primaryText") or " ",
            "image_hash": image_hash,
        }
        ...
        if body.get("cta"):
            link_data["call_to_action"] = {"type": body["cta"].upper()}

        creative_resp = requests.post(
            f"{GRAPH}/act_{ad_account_id}/adcreatives",
            params={"access_token": access_token},
            data={
                "name": f"SOC-Creative {int(now.timestamp())}",
        ...
        if creative_resp.get("error"):
            return jsonify({"ok": False, "stage": "creative", "details": creative_resp}), 500
        ...
        if ad_resp.get("error"):
            return jsonify({"ok": False, "stage": "ad", "details": ad_resp}), 500
        ...
        return jsonify({
            ...
            "creative_id": creative_id,
            "ad_id": ad_resp["id"],
            "used_page": selected_page,
        }), 200

    except Exception as e:
        current_app.logger.exception("publish_v2 failed")
        return jsonify({"ok": False, "error": str(e)}), 500

INTEREST_CACHE = {}  # keyword -> (timestamp, results)
INTEREST_CACHE_TTL = 60 * 60 * 24  # 24 hours


def search_facebook_interests(query: str, limit=5):
    if not query:
        return []
    q = str(query).strip().lower()
    now_ts = int(time.time())
    cached = INTEREST_CACHE.get(q)
    if cached and (now_ts - cached[0]) < INTEREST_CACHE_TTL:
        logger.debug("interest cache hit for %s", q)
        return cached[1]

    url = _graph_url("search")
    ...
    try:
        ...
        simplified = [{"id": x.get("id"), "name": x.get("name")} for x in results if x.get("id")]
        INTEREST_CACHE[q] = (now_ts, simplified)
        return simplified
    except Exception as e:
        logger.exception("search_facebook_interests failed for '%s': %s", query, e)
        return []
...
def save_inline_image(inline_data, prefix="ad"):
    """
    Save Gemini inline image data to disk and optionally upload to Spaces/S3.
    ...
    try:
        ...
        output_dir = OUTPUT_DIR  # make sure this exists in your project
        os.makedirs(output_dir, exist_ok=True)
        ...
        # Upload to Spaces/S3 if configured
        if "upload_to_spaces" in globals() and upload_to_spaces:
            _, uploaded = upload_to_spaces(local_path)
            public_url = uploaded.get("url") if uploaded else f"file://{local_path}"
        else:
        ...
        current_app.logger.exception("Failed to save inline Gemini image")
        return None, None
...
def generate_from_prodlink():
    """
    Gemini-only ad creative generator.
    Image includes ad copy rendered directly by Gemini.
    """
    ...
    try:
        ...
        snapshots = [
            ...
            if isinstance(u, str)
            and u.startswith("https://sociovia.blr1.cdn.digitaloceanspaces.com/snapshots/")
        ]
        ...
        for rs in ratio_specs:
            ...
            # Attach snapshot references
            for snap in snapshots:
                parts.append(Part.from_uri(file_uri=snap))

            creative_prompt = f"""
You are a senior creative designer creating a FINAL advertising image.

The output must be an IMAGE ONLY.

Design a premium SaaS marketing ad image with:
- Headline text embedded in the image
- CTA text embedded in the image

Text rules:
- Headline: short, bold, professional
- CTA: simple, action-oriented
...
"""
            ...
            parts.append(Part.from_text(text=creative_prompt))
            ...
            cfg = GenerateContentConfig(
                ...
                temperature=1.0,
                top_p=0.95,
                image_config=ImageConfig(aspect_ratio=rs["aspect"])
            )
            ...
            if resp and resp.candidates:
                for part in resp.candidates[0].content.parts:
                    if getattr(part, "inline_data", None):
                        fname, url = save_inline_image(
                            part.inline_data,
                            prefix=f"ad_{rs['aspect'].replace(':','')}"
                        )
                        if fname and url:
                            files.append(fname)
        ...
        current_app.logger.exception("generate-from-prodlink failed")
        return jsonify({
            "success": False,
            "error": "internal",
            "detail": str(e)
...
# Hardcoded fallback (you gave this earlier)
DEFAULT_STATIC_LEAD_FORM_ID = "4273874842858638"
# Default CTA type — must be one of FB allowed types. Change via env DEFAULT_CTA_TYPE if needed.
DEFAULT_CTA_TYPE = os.getenv("DEFAULT_CTA_TYPE", "LEARN_MORE")

# Updated facebook_adpreviews route — only send lead_gen_form_id (no form_id)
DEFAULT_STATIC_LEAD_FORM_ID = "4273874842858638"
DEFAULT_CTA_TYPE = os.getenv("DEFAULT_CTA_TYPE", "LEARN_MORE")
def _safe_serialize(obj: Any) -> Any:
    """Convert common non-serializable objects into JSON-friendly representations."""
    # requests.Response -> dict with status, text, url
    try:
        import requests as _req
        if isinstance(obj, _req.Response):
            return {
                "status_code": getattr(obj, "status_code", None),
                "url": getattr(obj, "url", None),
                "text": getattr(obj, "text", None)[:2000] if getattr(obj, "text", None) else None,
    ...
    # Fallback: try json.dumps; if fails, return str()
    try:
        json.dumps(obj)
        return obj
    except Exception:
...
def fetch_pages_for_ad_account(ad_account_id: str, access_token: str, api_version="v18.0"):
    """
    Fetch Facebook Pages connected to this ad account (Business Manager).
    """
    url = f"https://graph.facebook.com/{api_version}/act_{ad_account_id}/owned_pages"
    res = requests.get(
        url,
        params={"access_token": access_token},
        timeout=20,
...
def fetch_pages_for_user(access_token: str, api_version="v18.0"):
    """
    Fetch Facebook Pages the USER has access to.
    This is REQUIRED for ad previews.
    """
    ...
    return [
        {
            ...
            "name": p.get("name"),
            "category": p.get("category"),
            # page token is useful later (optional)
...
def normalize_image_for_meta(image_base64: str) -> bytes:
    # 1. Strip data URI
    image_base64 = re.sub(r"^data:image/.+;base64,", "", image_base64.strip())

    # 2. Remove whitespace/newlines (CRITICAL)
    ...
    img = img.convert("RGB")  # Meta-safe

    # 6. Re-encode PNG (NO optimize)
    output = BytesIO()
    img.save(
        output,
        format="PNG",
        optimize=False,        # ⚠️ VERY IMPORTANT
        compress_level=0       # Meta-safe
    )

    image_bytes = output.getvalue()

    # 7. Hard validation (prevents 8-byte bug)
    if len(image_bytes) < 10_000:
        raise Exception(f"Normalized image too small: {len(image_bytes)} bytes")

    # 8. PNG signature check
    if not image_bytes.startswith(b"\x89PNG\r\n\x1a\n"):
        raise Exception("Invalid PNG signature after normalization")
...
def upload_image_to_meta(image_bytes, ad_account_id, access_token, api_version):
    if len(image_bytes) < 10_000:
        raise Exception(f"Meta upload blocked: image too small ({len(image_bytes)} bytes)")

    url = f"https://graph.facebook.com/{api_version}/act_{ad_account_id}/adimages"

    files = {
        "file": ("image.png", image_bytes, "image/png")  # 🔥 FIX IS HERE
    }

    resp = requests.post(
        url,
        data={"access_token": access_token},  # token in data, NOT params
        files=files,
        timeout=30,
    )
    ...
    if not images:
        raise Exception("Meta returned no image hash")
...
def facebook_adpreviews():
    try:
        body = request.get_json(silent=True) or {}
        api_version = body.get("api_version", "v18.0")
        ...
        ad_account_id = fb_data["ad_account_id"]

        # 🔑 Ad previews REQUIRE user-owned pages
        pages = fetch_pages_for_user(access_token, api_version)
        if not pages:
        ...
        # ----------------------------
        # IMAGE HANDLING (HARDENED)
        # ----------------------------
        image_base64 = creative.get("image_base64")
        if not image_base64:
            return jsonify({"ok": False, "error": "image_base64_required"}), 400

        try:
            safe_image_bytes = normalize_image_for_meta(image_base64)

            current_app.logger.info(
                f"[META] normalized image bytes={len(safe_image_bytes)}"
            )
            ...
            current_app.logger.exception("Meta image upload failed")
            return jsonify({
                "ok": False,
                "error": "image_upload_failed",
                "details": str(e),
        ...
        if not link:
            return jsonify({"ok": False, "error": "creative.link_required"}), 400
        ...
        object_story_spec = {
            ...
            "link_data": {
                "link": link,
                "message": creative.get("body") or " ",
                "image_hash": image_hash,
            }
        }

        if cta_block:
            object_story_spec["link_data"]["call_to_action"] = cta_block

        res = requests.post(
            f"https://graph.facebook.com/{api_version}/act_{ad_account_id}/adcreatives",
            params={"access_token": access_token},
            data={
        ...
        # ----------------------------
        previews = []
        for fmt in body.get("ad_formats", ["MOBILE_FEED_STANDARD"]):
            r = requests.get(
                f"https://graph.facebook.com/{api_version}/{creative_id}/previews",
                params={
                    "access_token": access_token,
                    "ad_format": fmt,
                },
        ...
        current_app.logger.exception("facebook_adpreviews failed")
        return jsonify({"ok": False, "error": str(e)}), 500

import base64
import re

def upload_base64_image_to_meta(base64_str, ad_account_id, access_token, api_version):
    url = f"https://graph.facebook.com/{api_version}/act_{ad_account_id}/adimages"
    ...
    resp = requests.post(
        ...
        params={"access_token": access_token},
        data={
            "bytes": base64_str   # ⚠️ MUST be raw base64 only
        },
        timeout=30
    ).json()

    if resp.get("error"):
        print("META IMAGE ERROR:", resp)
        return None
    ...
    # Meta returns dict keyed by filename/hash
...
def generate_copy():
    ...
    if not workspace and workspace_id:
        try:
            ws_url = f"{APP_BASE_URL}/api/workspace?workspace_id={int(workspace_id)}"
    ...
    image_url = (
        ...
        or (
            body.get("selectedImages")[0]["url"]
            if body.get("selectedImages") else None
        )
    )
    ...
    # ----------------------
    prompt_lines = [
        "TASK: Generate 3 high-quality Meta Ad copy variations for TRAFFIC campaigns.",
        "Each variation must be premium, elegant, minimalist, global.",
        "",
        "Output must strictly be ONE JSON object with keys:",
        "raw_model_text, success=true, variations=[{id, primary_text, headline, description, cta, destination_url}]",
        "",
    ]
    ...
    # If product text is missing, instruct model to analyze the image
    if image_url and not (title or short_desc):
        prompt_lines.append("")
        prompt_lines.append("No textual data provided — analyze the image to infer:")
        prompt_lines.append("- product category")
        prompt_lines.append("- target audience")
        prompt_lines.append("- visual tone")
        prompt_lines.append("- style / mood")
        prompt_lines.append("Do NOT invent technical specs.")
    ...
    prompt_lines.append("")
    prompt_lines.append("Copywriting Requirements:")
    prompt_lines.append("- primary_text: 40-120 words (emotional + aspirational)")
    prompt_lines.append("- headline: <=40 chars")
    prompt_lines.append("- description: <=125 chars")
    prompt_lines.append("- CTA: Shop Now / Learn More / Discover More / Explore")
    prompt_lines.append("- destination_url: https://www.apple.com/")
    ...
    if image_url:
        try:
            img_resp = requests.get(image_url, timeout=6)
            if img_resp.status_code == 200:
                mime = img_resp.headers.get("Content-Type", "image/jpeg")
                parts.append(types.Part.from_bytes(data=img_resp.content, mime_type=mime))

                app.logger.info(f"✔ Image downloaded and added to LLM request ({mime})")
            else:
                app.logger.warning(f"⚠ Could not download image. Status {img_resp.status_code}")
        except Exception as e:
            app.logger.warning(f"⚠ Image fetch failed: {e}")

    # Confirm in logs that image part exists
    app.logger.info(
        f"LLM request parts: {len(parts)} (has_image={'yes' if len(parts) > 1 else 'no'})"
    )
    ...
    try:
        ...
        return jsonify({"success": False, "error": "genai_stream_failed"}), 502
    ...
    try:
        ...
        try:
            ...
            if start != -1 and end != -1:
                parsed = json.loads(accumulated[start : end + 1])
            else:
                raise
        except Exception:
            app.logger.exception("invalid_model_output")
            return jsonify({
                "success": False,
                "error": "invalid_model_output",
                "raw_model_output": accumulated,
            }), 502
    ...
    for i, item in enumerate(variations[:count]):
        ...
        desc = ""
        cta = "Learn More"
        dest = workspace.get("website") if workspace else "https://www.apple.com/"

        if isinstance(item, dict):
            vid = item.get("id") or vid
            primary = item.get("primary_text") or item.get("primaryText") or item.get("text") or ""
            head = item.get("headline") or item.get("title") or ""
            desc = item.get("description") or item.get("desc") or ""
            cta = item.get("cta") or item.get("call_to_action") or "Learn More"
            dest = item.get("destination_url") or item.get("destinationUrl") or item.get("url") or dest

        elif isinstance(item, str):
            # handle bare string failure cases from model
            primary = item
            head = (item[:40] + "...") if len(item) > 40 else item
            desc = (item[:100] + "...") if len(item) > 100 else item
    ...
    if not normalized:
        normalized = [
            {
                "id": "1",
                "primary_text": short_desc or "Discover this product",
                "headline": f"Discover {title or 'Product'}",
                "description": (short_desc or "Explore more on our site")[:125],
                "cta": "Learn More",
                "destination_url": workspace.get("website") if workspace else "https://www.apple.com/",
            }
        ]
...
# ============================================================
# App Owner Notification Settings (Hardcoded for Waitlist Alerts)
# ============================================================
APP_OWNER_NOTIFICATION = {
    "waba_id": "1328898605704972",
    "phone_number_id": "909603232241870",
    "cofounder_phone": "919986849887",  # Co-founder's phone number to receive notifications
}


def send_waitlist_notification(name: str, email: str, phone: Optional[str], workspace_id: str = None):
    """
    Send WhatsApp notification to co-founder when new user joins waitlist.
    Uses HARDCODED app owner credentials - independent of user's linked WhatsApp account.
    
    Template: sociovia_onboard
    Variables: Name, Email, Phone, Workspace/Brand, Signup time
    """
    try:
        ...
        # Use the app owner's WhatsApp account (hardcoded credentials)
        account = WhatsAppAccount.query.filter_by(
            phone_number_id=APP_OWNER_NOTIFICATION["phone_number_id"],
            is_active=True
        ).first()
        
        if not account:
            logger.warning(f"App owner WhatsApp account not found (phone_number_id: {APP_OWNER_NOTIFICATION['phone_number_id']})")
            return
        
        access_token = account.get_access_token()
        if not access_token:
            logger.warning(f"No access token for app owner account {account.id}")
            return
        
        # Create WhatsApp service with app owner's credentials
        ...
        body_params = [
            ...
            "Waitlist Signup",  # Workspace/Brand - using placeholder since it's a waitlist
            signup_time
        ]
        
        # Send template message to co-founder
        result = service.send_template(
            to=APP_OWNER_NOTIFICATION["cofounder_phone"],
        ...
        if result.get("success"):
            logger.info(f"✅ Sent waitlist notification for {email} to co-founder ({APP_OWNER_NOTIFICATION['cofounder_phone']})")
        else:
            logger.warning(f"⚠️ Failed to send waitlist notification: {result.get('error')}")
            
    except Exception as e:
        # Don't fail the waitlist signup if notification fails
        logger.exception(f"Failed to send waitlist notification: {e}")
...
SERVICE_ACCOUNT_FILE = "C:\\Users\\ASUS\\OneDrive\\Desktop\\sociovia-28-11-2025\\Sociovia\\angular-sorter-473216-k8-835a67a5574c.json"

SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]
def get_sheets_service(timeout_sec: int = 30):
    """
    Build Google Sheets client using service-account JSON file.
    - Validates JWT immediately
    - Uses custom HTTP timeout
    - Safe for background workers & prod
    """
    try:
        raw_http = httplib2.Http(timeout=timeout_sec)

        # ✅ Load credentials from JSON file (known-good)
        credentials = service_account.Credentials.from_service_account_file(
            SERVICE_ACCOUNT_FILE,
            scopes=SCOPES,
        )

        # 🔍 Fail fast if JWT / key is invalid
...
def api_waitlist():
    ...
    try:
        ...
        # Send WhatsApp notification to co-founder
        try:
            send_waitlist_notification(name, email, phone)
        except Exception:
            logger.exception("Failed to send WhatsApp notification for waitlist signup; continuing")
...
def _load_response_json(conv):
    """
    Try to load conversation.response field to python object.
    Always returns a dict that can be modified safely.
    """
    try:
        if getattr(conv, "response", None):
            parsed = json.loads(conv.response)
            # If the response is a list, wrap it so we can add metadata
            if isinstance(parsed, list):
                return {"_data": parsed, "_is_list": True}
            elif isinstance(parsed, dict):
                return parsed
            else:
                return {"_data": parsed}
    except Exception:
        current_app.logger.debug("Could not parse conversation.response; returning empty dict")
    return {}

def _save_response_json(conv, obj):
    """
    Save the response JSON back to the conversation.
    """
    try:
        conv.response = json.dumps(obj)
        current_app.logger.debug("Saved response JSON for conv %s: %s", getattr(conv, "id", "?"), conv.response[:200] if conv.response else "empty")
    except Exception as e:
        ...
        if hard:
            ...
            try:
                if hasattr(conv, "deleted"):
                    ...
                    current_app.logger.info("Soft-delete: loading response JSON for conv %s", conv_id)
                    resp = _load_response_json(conv)
                    current_app.logger.info("Soft-delete: loaded response type=%s, keys=%s", type(resp).__name__, list(resp.keys()) if isinstance(resp, dict) else "N/A")
                    resp["_deleted"] = True
                    resp["_deleted_at"] = datetime.utcnow().isoformat()
                    current_app.logger.info("Soft-delete: saving response with _deleted=True for conv %s", conv_id)
                    _save_response_json(conv, resp)
                    current_app.logger.info("Soft-delete: saved, response now = %s", conv.response[:300] if conv.response else "empty")

                db.session.add(conv)
                db.session.commit()
                current_app.logger.info("Soft-deleted conversation %s by user %s - commit successful", conv_id, user_id)
    ...
    try:
        ...
        try:
            if hasattr(conv, "title"):
                ...
                current_app.logger.info("Rename: Loading response JSON for conv %s", conv_id)
                resp = _load_response_json(conv)
                current_app.logger.info("Rename: Loaded response, type=%s, keys=%s", type(resp).__name__, list(resp.keys()) if isinstance(resp, dict) else "N/A")
                ...
                current_app.logger.info("Rename: Saving response with _renamed_to=%s", new_title)
                _save_response_json(conv, resp)
                current_app.logger.info("Rename: After save, response=%s", conv.response[:300] if conv.response else "empty")
                
                db.session.add(conv)
                db.session.commit()
                current_app.logger.info("Rename: Committed to DB for conv %s by user %s", conv_id, user_id)
                
                # Verify by re-fetching
                db.session.refresh(conv)
                current_app.logger.info("Rename: After refresh, response=%s", conv.response[:300] if conv.response else "empty")
...
SYSTEM_PROMPT = f"""You are Sociovia AI, an expert growth & CRM assistant.

### CORE INSTRUCTIONS:
1. **BE ULTRA-CONCISE**: Do not use filler words, pleasantries (e.g., "I hope you are well"), or long introductions. Get straight to the point.
2. **USE STRUCTURE**: Always use **Bullet Points**, **Bold Text**, or **Tables** to present information. Avoid wall-of-text paragraphs.
3. **RELEVANCE ONLY**: Only reference the User, Workspace, or Competitors if it is directly necessary to answer the specific question. Do not summarize the workspace unless asked.
4. **DATA HANDLING**: 
   - If analytics.has_real_analytics is False, clearly state: "Metrics are currently simulated (0)." Do not explain *why* unless asked.
   - Proceed with requested workflows/simulations immediately.

### FORMATTING RULES:
- **Good**: "Here is the summary:\n• ROAS: 0 (Simulated)\n• Spend: 0"
- **Bad**: "Sure, I can help you with that. Based on the data available in your workspace for [Business Name]..."

### CONTEXT HANDLING:
- You have access to the user's business details. Use them implicitly to tailor advice (e.g., if B2B, suggest LinkedIn; if B2C, suggest Instagram) without restating the details to the user.
- If the user asks for support, point them to {SUPPORT_EMAIL}.
"""
...
def build_prompt_with_context(
    ...
    # --- 🔥 CRITICAL ADDITION: FINAL INSTRUCTION ---
    lines.append("### FINAL INSTRUCTION:")
    lines.append("Answer the last user message above. Be concise. Use structural formatting (bullets/bold). Answer ONLY what was asked.")
...
def generate_ai_reply(
    ...
    try:
        # Add config to limit verbosity
        from google.genai import types 
        
        config = types.GenerateContentConfig(
            max_output_tokens=300,  # Limit length (approx 200 words)
            temperature=0.3         # Lower temperature = less "creative" fluff, more direct
        )
...
def _label_to_value(label: str) -> str:
    """Sanitize a label to a short snake_case value (fallback to "option")."""
...
ALLOWED_THANK_YOU_BUTTON_TYPES = {
    "VIEW_WEBSITE", "CALL_BUSINESS", "MESSAGE_BUSINESS", "DOWNLOAD",
    "SCHEDULE_APPOINTMENT", "VIEW_ON_FACEBOOK", "PROMO_CODE", "NONE",
    "WHATSAPP", "P2B_MESSENGER", "BOOK_ON_WEBSITE"
}

def _normalize_thankyou_button(bt: Optional[str]) -> str:
    """Ensure button type is allowed; fallback to NONE."""
    if not bt:
        return "NONE"
    bt = str(bt).upper()
    return bt if bt in ALLOWED_THANK_YOU_BUTTON_TYPES else "NONE"

# -------------------------
# Stubbed model helpers (replace with real implementations)
# -------------------------
def _generate_text_from_prompt(prompt: str, model_id: str = "gemini-flash-latest", **kwargs) -> Any:
    """
    Stubbed model call: in your environment replace this with real helper that returns an object
    where _extract_text_from_model_resp can pull text and usage metadata is present.
    For offline/development we return a JSON string of a mock lead_form.
    """
    # Mock JSON text: a simple friendly lead_form object (no wrapping "lead_form" key)
    mock = {
        "form_name": "Acme Coaching - Free Trial",
        "intro_text": "Get a free trial class — tell us a bit about yourself and your goals.",
        ...
        "questions": [
            ...
            {"id": "phone", "type": "PHONE", "label": "Phone number", "required": False, "placeholder": "+91-XXXXXXXXXX"},
            {"id": "course_interest", "type": "MCQ_SINGLE", "label": "Which course are you interested in?", "required": True,
             "options": [{"label": "Data Science & ML"}, {"label": "Web Development"}, {"label": "Android Development"}]},
            {"id": "budget_range", "type": "DROPDOWN", "label": "Budget range", "required": False,
             "options": [{"label": "Under ₹10,000"}, {"label": "₹10,000 - ₹25,000"}, {"label": "Above ₹25,000"}]},
            {"id": "preferred_start_date", "type": "DATE", "label": "Preferred start date", "required": False},
            {"id": "subscribe_updates", "type": "BOOLEAN", "label": "Subscribe for course updates and offers?", "required": False},
            {"id": "tell_us_more", "type": "TEXTAREA", "label": "Tell us about your goals (brief)", "required": False,
             "placeholder": "E.g. Preparing for internship, want to learn ML basics..."}
        ],
        "confidence": 0.78
    }
    # Wrap as simple object with text + usage metadata shape expected by our extractors.
    class Resp:
        pass
    r = Resp()
    r.text = json.dumps(mock)
    r.usage_metadata = {"prompt_token_count": 10, "candidates_token_count": 120, "total_token_count": 130}
    return r

def _extract_text_from_model_resp(resp) -> Optional[str]:
    # Try common fields for model response
    if resp is None:
        return None
    if hasattr(resp, "text"):
...
def calculate_cost_inr(model_name: str, input_tokens: int, output_tokens: int) -> Decimal:
    # Stub cost: ₹0.0005 per token total (example)
    per_token = Decimal("0.0005")
    return per_token * Decimal(input_tokens + output_tokens)

def log_ai_usage(**kwargs) -> Optional[int]:
    # Stub logger: print and return fake id
    logger.info("log_ai_usage called: %s", json.dumps({k: v for k, v in kwargs.items() if k != "raw_response"}, default=str))
    return int(time.time())  # fake id

# -------------------------
# Core generator function
# -------------------------
def generate_leadform_from_workspace(
    workspace: Dict[str, Any],
    model_id: Optional[str] = None,
    user_prompt: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Generate a lead form JSON (friendly internal form) using an LLM,
    then produce a Meta (Facebook) Graph-API compatible meta_payload
    derived from that lead_form.

    Returns a dict: { ok, source, model, lead_form, meta_payload, meta_compatibility_note,
                      confidence, raw_text, usage, error? }
    """
    if model_id is None:
        model_id = "gemini-flash-latest"

    # make workspace JSON safe
    try:
        workspace_json = json.dumps(workspace, ensure_ascii=False, indent=2)
    except Exception:
        def _safe(obj):
            try:
                ...
                return str(obj)
        workspace = {k: _safe(v) for k, v in (workspace or {}).items()}
        workspace_json = json.dumps(workspace, ensure_ascii=False, indent=2)

    user_prompt_snippet = (user_prompt or "").strip()

    # Prompt the model to return a compact lead_form JSON only.
    prompt = f"""You MUST output ONLY a single JSON object (no explanation).
Return a compact "lead_form" object only.
...
""".strip()
    ...
    def _safe_parse_json(text: str) -> Optional[dict]:
        """Try to extract and parse the first JSON object from model text."""
    ...
    def _make_meta_custom(q: Dict[str, Any], note_items: list) -> Dict[str, Any]:
        mq = {"type": "CUSTOM", "label": (q.get("label") or q.get("id") or "Question")[:250]}
        qtype = (q.get("type") or "").upper()
        if qtype in ("MCQ_SINGLE", "MCQ_MULTI", "DROPDOWN"):
            mq["style"] = "SELECT"
            raw_opts = q.get("options") or []
            cleaned = []
            for opt in raw_opts:
                if isinstance(opt, dict):
                    lab = str(opt.get("label") or opt.get("value") or "").strip()
                    val = opt.get("value") or None
                else:
                    lab = str(opt).strip()
                    val = None
                if not lab:
                    continue
                if not val:
                    val = _label_to_value(lab)
                cleaned.append({"label": lab[:250], "value": str(val)[:64]})
            if not cleaned:
                cleaned = [{"label": "Option 1", "value": "option_1"}, {"label": "Option 2", "value": "option_2"}]
                note_items.append(f"Added fallback options for question '{mq['label']}' because model provided none.")
            mq["options"] = cleaned
        else:
            mq["style"] = "TEXT"
        return mq

    def _map_to_meta_payload(lead_form: Dict[str, Any], workspace_obj: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
        note_items = []
        lf = deepcopy(lead_form or {})

        # normalize locale
        page_locale = (lf.get("locale") or workspace_obj.get("locale") or "EN_US")
        page_locale = str(page_locale).upper().replace('-', '_')

        # Create a unique preview-friendly name (append short timestamp) to avoid name collisions
        base_name = lf.get("form_name") or workspace_obj.get("business_name") or "Lead Form"
        ts = int(time.time())
        unique_name = f"{base_name} - preview-{ts % 100000}"

        meta = {
            "name": unique_name,
            "privacy_policy_url": lf.get("privacy_policy_url") or "",
            "privacy_policy": {"url": lf.get("privacy_policy_url") or "", "link_text": "Privacy Policy"},
            "locale": page_locale,
            "questions": [],
            "thank_you_page": {
                "title": lf.get("thank_you_text") or "Thank you",
                "body": lf.get("intro_text") or "",
                "button_type": "NONE",
                "button_text": "",
                "button_url": "",
            },
        ...
        website = (workspace_obj.get("website") or workspace_obj.get("site") or workspace_obj.get("object_url") or "").strip()
        if website:
            meta["thank_you_page"]["button_type"] = _normalize_thankyou_button("VIEW_WEBSITE")
            meta["thank_you_page"]["button_url"] = website
            meta["thank_you_page"]["button_text"] = "Visit website"
        else:
            meta["thank_you_page"]["button_type"] = _normalize_thankyou_button(meta["thank_you_page"]["button_type"])
        ...
        builtin_map = {
            ...
            "WHATSAPP_NUMBER": {"type": "WHATSAPP_NUMBER"},
        }

        questions = lf.get("questions") or []
        count = 0
        for q in questions:
            ...
            if q_type in builtin_map:
                meta_q = {"type": builtin_map[q_type]["type"]}
                meta["questions"].append(meta_q)
                if q.get("required") or q.get("placeholder") or q.get("label"):
                    note_items.append(
                        f"Stripped label/required/placeholder from Meta built-in '{meta_q['type']}' (question id: {q.get('id')})."
                    )
            else:
                mq = _make_meta_custom(q, note_items)
                meta["questions"].append(mq)

            count += 1

        if not meta["questions"]:
            meta["questions"] = [{"type": "EMAIL"}]
            note_items.append("No questions generated; added fallback EMAIL question.")

        bt = meta["thank_you_page"].get("button_type")
        if bt not in ALLOWED_THANK_YOU_BUTTON_TYPES:
            note_items.append(f"Adjusted thank_you_page.button_type from '{bt}' to 'NONE' because it was not allowed.")
            meta["thank_you_page"]["button_type"] = "NONE"
            meta["thank_you_page"]["button_text"] = ""
            meta["thank_you_page"]["button_url"] = ""

        return meta, "; ".join(note_items)[:1024]
    ...
    try:
        if callable(globals().get("_generate_text_from_prompt")):
            model_used = model_id
            resp = _generate_text_from_prompt(prompt, model_id=model_id, response_modalities=["TEXT"], candidate_count=1)
            raw_text = _extract_text_from_model_resp(resp)
            parsed = _safe_parse_json(raw_text)
            usage = _extract_usage(resp)
        else:
            # no model client available - fallback to a reasonable mock lead_form using workspace
            model_used = None
            parsed = None

        # Fallback: if parsed is None, synthesize a simple lead_form using workspace + user_prompt
        if not parsed:
            # Create a minimal friendly lead_form (fallback)
            biz = workspace or {}
            name_hint = biz.get("business_name") or biz.get("name") or "My Business"
            mock_lead = {
                "form_name": f"{name_hint} - Contact",
                "intro_text": biz.get("description") or "Please tell us about yourself so we can get back to you.",
                "privacy_policy_url": biz.get("privacy_policy_url") or "",
                "locale": biz.get("locale") or "en_US",
                "thank_you_text": "Thanks — we'll be in touch!",
                "questions": [
                    {"id": "full_name", "type": "FULL_NAME", "label": "Full name", "required": True},
            ...
            # If user provided a prompt that requests specific fields, try to honor it (very simple)
            if user_prompt_snippet and "budget" in user_prompt_snippet.lower():
                mock_lead["questions"].append({
                    "id": "budget_range",
                    "type": "DROPDOWN",
                    "label": "Budget range",
                    "required": False,
        ...
        # Accept either { "lead_form": {...} } or the model returned the form root directly
        if parsed and isinstance(parsed, dict) and "lead_form" in parsed and isinstance(parsed["lead_form"], dict):
            lead_form = parsed["lead_form"]
        elif parsed and isinstance(parsed, dict) and "questions" in parsed and "form_name" in parsed:
            lead_form = parsed
        else:
            return {"ok": False, "error": "Model returned unparsable JSON or missing lead_form", "model": model_used, "raw_text": raw_text, "usage": usage}

        if not isinstance(lead_form, dict):
            return {"ok": False, "error": "Parsed lead_form not an object", "model": model_used, "raw_text": raw_text, "usage": usage}

        # Normalise and clean lead_form
        lead_form.setdefault("form_name", str(lead_form.get("form_name") or workspace.get("business_name") or "Lead Form"))
        lead_form.setdefault("intro_text", str(lead_form.get("intro_text") or "Please fill out this form."))
        lead_form.setdefault("privacy_policy_url", str(lead_form.get("privacy_policy_url") or ""))
        lead_form.setdefault("locale", str(lead_form.get("locale") or workspace.get("locale") or "EN_US"))
        lead_form.setdefault("thank_you_text", str(lead_form.get("thank_you_text") or "Thank you!"))

        qlist = lead_form.get("questions") or []
        cleaned = []
        for i, q in enumerate(qlist or []):
            if not isinstance(q, dict):
                continue
            qid = q.get("id") or q.get("key") or f"q_{i+1}"
            qid = re.sub(r'[^a-z0-9_]+', '_', str(qid).strip().lower()).strip('_') or f"q_{i+1}"
            qtype = (q.get("type") or "").upper() or "TEXT"

            options = []
            if qtype in ("MCQ_SINGLE", "MCQ_MULTI", "DROPDOWN"):
                raw_opts = q.get("options") or []
                for j, op in enumerate(raw_opts or []):
                    if isinstance(op, dict):
                        lab = str(op.get("label") or op.get("value") or "").strip()
                        val = op.get("value") or None
                    else:
                        lab = str(op).strip()
                        val = None
                    if not lab:
                    ...
                    if not val:
                        val = _label_to_value(lab)
                    options.append({"value": str(val)[:64], "label": lab[:250]})
                if not options:
            ...
            cleaned_q = {
                "id": qid,
                "type": qtype,
                "label": str(q.get("label") or q.get("title") or qid)[:60],
                "required": bool(q.get("required") is True),
            }
            if q.get("placeholder"):
                cleaned_q["placeholder"] = str(q.get("placeholder"))[:200]
            if options:
                cleaned_q["options"] = options
            if q.get("validation"):
                cleaned_q["validation"] = q.get("validation")
            if q.get("conditional"):
                cleaned_q["conditional"] = q.get("conditional")
            if q.get("ui_hint"):
                cleaned_q["ui_hint"] = q.get("ui_hint")
            cleaned.append(cleaned_q)

        lead_form["questions"] = cleaned

        # Build meta_payload
        meta_payload, meta_note = _map_to_meta_payload(lead_form, workspace)

        result = {
            "ok": True,
            "source": "ai+server" if model_used else "fallback",
            "model": model_used,
            "lead_form": lead_form,
            "meta_payload": meta_payload,
            "meta_compatibility_note": meta_note,
            "confidence": float(parsed.get("confidence", 0.0) or 0.0) if isinstance(parsed.get("confidence", None), (int, float, str)) else 0.0,
            "raw_text": raw_text,
            "usage": usage,
        }
        return result
        ...
        logger.exception("generate_leadform_from_workspace failed: %s", e)
        return {"ok": False, "error": f"Model call or mapping failed: {e}", "model": model_used, "raw_text": raw_text, "usage": {"input_tokens":0,"output_tokens":0,"total_tokens":0}}

# -------------------------
# AI generate routes + usage logging helper
# -------------------------
def _compute_cost_and_log(result: dict, feature: str, workspace_id: Optional[int], user_id: Optional[int], route_path: str):
    usage = result.get("usage") or {}
    ...
    cost_inr = 0.0
    try:
        cost_inr_dec = calculate_cost_inr(model_name, input_tokens, output_tokens) if callable(globals().get("calculate_cost_inr")) else Decimal("0")
        cost_inr = float(cost_inr_dec)
    except Exception as e:
    ...
    try:
        if callable(globals().get("log_ai_usage")):
            saved_id = log_ai_usage(
                ...
                ip_address=request.remote_addr if request is not None else None,
                raw_response=result.get("raw_text") or None,
            )
            if saved_id:
                result["ai_usage_id"] = saved_id
        else:
            logger.info("AI usage (no DB): user=%s workspace=%s model=%s usage=%s cost=₹%s", user_id, workspace_id, model_name, usage, cost_inr)
    except Exception as e:
        logger.exception("Failed to persist AI usage: %s", e)
...
@app.route("/api/workspace/<workspace_id>/ai-suggest-leadform", methods=["POST"])
def ai_suggest_leadform_route(workspace_id):
    try:
        payload = request.get_json(force=True, silent=True) or {}
        workspace = payload.get("workspace") or {}
        user_prompt = (payload.get("user_prompt") or payload.get("prompt") or "").strip()
        workspace["id"] = workspace.get("id") or workspace_id
        ...
        if callable(globals().get("generate_leadform_from_workspace")):
            result = generate_leadform_from_workspace(
                workspace,
                model_id=globals().get("TEXT_MODEL") if 'TEXT_MODEL' in globals() else None,
                user_prompt=user_prompt or None
            )
        else:
            return jsonify({"ok": False, "error": "LeadForm generator not available on server."}), 500

        user_id = None
        try:
            # replace with your real current user resolver if available
            if callable(globals().get("get_current_user_id_safe")):
                user_id = get_current_user_id_safe()
        except Exception:
            user_id = None
        if user_id is None:
        ...
        _compute_cost_and_log(result, feature="ai_suggest_leadform", workspace_id=_safe_int(workspace_id), user_id=user_id, route_path=request.path)

        return jsonify(result), 200

    except Exception as e:
        logger.exception("ai_suggest_leadform error: %s", e)
        return jsonify({"ok": False, "error": str(e)}), 500

# Compatibility shim for older frontend path
@app.route("/api/ai/generate-form", methods=["POST"])
def ai_generate_form_compat():
    try:
        payload = request.get_json(silent=True) or {}
        workspace_id = request.args.get("workspace_id") or payload.get("workspace", {}).get("id")
        user_id_q = request.args.get("user_id") or payload.get("user_id")
        ...
        if callable(globals().get("generate_leadform_from_workspace")):
            result = generate_leadform_from_workspace(workspace, model_id=globals().get("TEXT_MODEL") if 'TEXT_MODEL' in globals() else None, user_prompt=user_prompt or None)
        else:
            return jsonify({"ok": False, "error": "LeadForm generator not available on server."}), 500

        user_id = user_id_q or workspace.get("user_id")
        _compute_cost_and_log(result, feature="ai_suggest_leadform_compat", workspace_id=_safe_int(workspace_id), user_id=user_id, route_path=request.path)

        return jsonify(result), 200

    except Exception as e:
        logger.exception("ai_generate_form_compat error: %s", e)
        return jsonify({"ok": False, "error": str(e)}), 500

# -------------------------
# Debug endpoint to print payloads for testing / reproduction
# -------------------------
@app.route("/api/debug/mock-payload", methods=["POST", "GET"])
def debug_mock_payload():
   
    try:
        payload = request.get_json(silent=True) or {}
        workspace = payload.get("workspace") or {
            "business_name": "Acme Coaching",
            "description": "Test provider of tech courses",
        ...
        result = generate_leadform_from_workspace(workspace, model_id=None, user_prompt=user_prompt)
        # Return both lead_form and meta_payload for manual inspection
        return jsonify({
            "ok": True,
            "lead_form": result.get("lead_form"),
            "meta_payload": result.get("meta_payload"),
            "meta_compatibility_note": result.get("meta_compatibility_note"),
            "raw_text": result.get("raw_text")
        }), 200
    except Exception as e:
        logger.exception("debug_mock_payload error: %s", e)
        return jsonify({"ok": False, "error": str(e)}), 500
...
# ---------- FIX TYPE ----------
def sanitize_question_type(t: str) -> Optional[str]:
    if not t:
        return "CUSTOM"

    t = t.upper().strip()
    if t in ALLOWED_QUESTION_TYPES:
        return t
...
# ---------- FIX SHAPE ----------
def ensure_questions_shape(questions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out = []
    for q in questions:
        if not isinstance(q, dict):
            continue

        qtype = sanitize_question_type(
            q.get("type") or q.get("question_type") or q.get("hint") or "CUSTOM"
        )

        label = q.get("label") or q.get("text") or q.get("question") or "Question"
        ...
        # FULL_NAME / EMAIL / PHONE (Meta will reject if label is provided)
        if qtype in ("FULL_NAME", "EMAIL", "PHONE"):
            pass
        ...
        if qtype == "MULTIPLE_CHOICE":
            ...
            if isinstance(opts, str):
                try:
                    ...
                    opts = [opts]
            opts_clean = [{"label": str(o)[:80]} for o in opts if o]
            if len(opts_clean) < 2:
                qobj["question_type"] = "SHORT_TEXT"
            else:
                qobj["options"] = opts_clean[:10]

        # CUSTOM QUESTIONS REQUIRE KEY
        if qtype == "CUSTOM":
            qobj["key"] = re.sub(r'[^a-z0-9_]', '_', label.lower())[0:40]
...
# ---------- GENERATE TEXT ----------
def _generate_text_from_prompt(prompt, model_id=None, temperature=0.2, max_output_tokens=512, response_modalities=None, candidate_count=1):
    client = GENAI_CLIENT
    prompt = json.dumps(prompt, indent=2) if isinstance(prompt, (dict, list)) else str(prompt)
    resp = client.models.generate_content(model=model_id or TEXT_MODEL, contents=prompt)
    return resp


# ---------- PROMPT BUILDER ----------
def build_prompt_for_gemini(payload: Dict[str, Any]) -> str:
    ws = payload.get("workspace", {})
    business_name = payload.get("business_name") or ws.get("name") or "Your Business"

    static = """
Rules:
1) Output ONLY valid JSON.
2) JSON must include: name, locale, questions.
3) Question fields must follow EXACT Meta schema:
   - question_type: FULL_NAME | EMAIL | PHONE | SHORT_TEXT | LONG_TEXT | MULTIPLE_CHOICE | YES_NO | CUSTOM
   - For CUSTOM: include "key"
   - For MULTIPLE_CHOICE: options = [{ "label": "..." }]
4) context_card uses: { "title": "...", "content": "..." }
5) thank_you_page uses: { "title", "body", "redirect_url" }
6) No markdown or text outside JSON.
...
# ---------- API ROUTE ----------
@app.route("/api/ai/generate-form", methods=["POST"])
@app.route("/api/ai/generate-form/<workspace_id>", methods=["POST"])
@rate_limit("text_generate")
def generate_lead_form(workspace_id=None):
    incoming = request.get_json(silent=True) or {}

    prompt = build_prompt_for_gemini(incoming)

    try:
        resp = _generate_text_from_prompt(prompt, model_id=TEXT_MODEL, temperature=0.0, max_output_tokens=1024)
        text = resp.text
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Gemini did not return JSON", "gemini_output": text}), 500

    # ----- FIX QUESTIONS -----
    questions = ensure_questions_shape(parsed.get("questions", []))

    if not questions:
        return jsonify({"ok": False, "error": "AI returned no valid questions", "raw": text}), 500

    # FINAL RESPONSE
    out = {
        "name": parsed.get("name", "Lead Form"),
        "locale": parsed.get("locale", "en_US"),
        "questions": questions
    }

    if parsed.get("privacy_policy_url"):
        out["privacy_policy"] = {"url": parsed["privacy_policy_url"]}

    if parsed.get("follow_up_action_url"):
        out["follow_up_action_url"] = parsed["follow_up_action_url"]

    if parsed.get("context_card"):
        cc = parsed["context_card"]
        out["context_card"] = {
            "title": cc.get("title", ""),
            "content": cc.get("body") or cc.get("content") or ""
        }

    if parsed.get("thank_you_page"):
        out["thank_you_page"] = parsed["thank_you_page"]

    return jsonify({"ok": True, "form": out, "raw_gemini": text}), 200
...
@app.route("/api/meta/forms/create", methods=["POST"])
def create_meta_lead_form():
    ...
    thank_you = data.get("thank_you")
    privacy = data.get("privacy_policy")
    fields = data.get("fields", [])

    if not name or not fields:
        return jsonify({"error": "Form name and at least one question required"}), 400
    if not privacy:
        return jsonify({"error": "privacy_policy URL is required"}), 400

    social = SocialAccount.query.filter_by(user_id=str(user.id), provider="facebook").first()
    if not social:
        return jsonify({"error": "No connected Facebook Page"}), 400

    page = social.provider_user_id
    ...
    for q in fields:
        ...
        if not q_type:
            continue
        sanitized_fields.append({"type": q_type} if q_type != "CUSTOM" else q)

    # Wrap privacy_policy
    privacy_obj = {"url": privacy, "link_text": "Privacy Policy"} if isinstance(privacy, str) else privacy

    # Derive follow-up URL
    follow_up_url = privacy_obj.get("url", "").rstrip("/")
    if follow_up_url.endswith("/privacy"):
        follow_up_url = follow_up_url[: -len("/privacy")]

    payload = {
        "name": name,
        "questions": sanitized_fields,
        "privacy_policy": privacy_obj,
        "privacy_policy_url": privacy_obj.get("url"),
        "intro_text": intro,
        "thank_you_text": thank_you,
        # ✅ Correct follow-up action
        "follow_up_action_url": follow_up_url or None
    }

    try:
        url = f"https://graph.facebook.com/v21.0/{page}/leadgen_forms"
        headers = {"Content-Type": "application/json"}
        r = requests.post(url, params={"access_token": token}, json=payload, headers=headers).json()

        if "id" not in r:
            return jsonify({"error": r.get("error", "Meta API failed")}), 400

        view_url = f"https://www.facebook.com/leadgen_form_preview/{r['id']}"
        return jsonify({
            "ok": True,
            "formId": r["id"],
            "viewUrl": view_url
...
@app.route("/api/meta/forms", methods=["GET", "OPTIONS"])
def list_forms():
    workspace_id = request.args.get("workspace_id")
    user_id = request.args.get("user_id")
    ...
    # workspace_id is STRING in DB → must cast
    social = SocialAccount.query.filter_by(
        user_id=str(user_id),      # if this is also text
        workspace_id=str(workspace_id),
        provider="facebook"
    ).first()

    if not social:
        return jsonify({"error": "No linked Facebook Page"}), 400

    page_id = social.provider_user_id
...
@app.route("/api/meta/forms/<form_id>/preview", methods=["GET"])
def preview_meta_form(form_id):
    return jsonify({
        "form_id": form_id,
        "url": f"https://www.facebook.com/leadgen_form_preview/{form_id}"
    })
...
try:
    ...
    print("[scheduler] APScheduler not installed. Background cleanup disabled.")


def cleanup_expired_images_job():
    """
    Background job to clean up expired images.
    Runs daily to:
    1. Find all expired images (expires_at <= NOW() AND is_permanent = FALSE)
    2. Delete the actual image files from storage (S3/Spaces)
    ...
    with app.app_context():
        try:
            ...
            expired_images = SavedImage.query.filter(
                SavedImage.is_permanent == False,
                SavedImage.expires_at != None,
                SavedImage.expires_at <= now
            ).all()
            ...
            if not expired_images:
                print(f"[cleanup_job] No expired images found at {now.isoformat()}")
                return
            ...
            for img in expired_images:
                try:
                    ...
                    if img.saved_key:
                        try:
                            s3.delete_object(Bucket=SPACE_NAME, Key=img.saved_key)
                            print(f"[cleanup_job] Deleted {img.saved_key} from storage")
                        except Exception as e:
                            print(f"[cleanup_job] Failed to delete {img.saved_key} from storage: {e}")
                    
                    # Delete from SavedImage table
                    db.session.delete(img)
                    ...
                    print(f"[cleanup_job] Error deleting image {img.id}: {e}")
            
            db.session.commit()
            print(f"[cleanup_job] Cleaned up {deleted_count} expired images at {now.isoformat()}")
            
        except Exception as e:
            print(f"[cleanup_job] Job failed: {e}")
            db.session.rollback()


def send_expiration_warnings_job():
    """
    Background job to send email notifications for images expiring soon.
    Runs daily to notify users about images expiring in the next 3 days.
    """
    with app.app_context():
        try:
            now = datetime.now(timezone.utc)
            ...
            # Get images expiring in 3 days that haven't been notified
            # Group by user
            from sqlalchemy import func
            
            expiring_users = db.session.query(
                SavedImage.user_id,
                func.count(SavedImage.id).label('image_count')
            ).filter(
                SavedImage.is_permanent == False,
                SavedImage.expires_at != None,
                SavedImage.expires_at > now,
                SavedImage.expires_at <= warning_threshold,
                SavedImage.expiry_notified == False
            ).group_by(SavedImage.user_id).all()
            
            for user_id, image_count in expiring_users:
                try:
                    # Get user email
                    user = User.query.filter_by(id=int(user_id)).first()
                    if user and user.email:
                        # Send email notification
                        subject = "Your saved images will expire soon"
                        body = f"""
Hello,

You have {image_count} image(s) that will expire in the next 3 days.

To keep these images permanently, please log in to Sociovia and save them:
...
"""
                        try:
                            ...
                            print(f"[expiry_warning] Sent notification to {user.email} for {image_count} images")
                        except Exception as e:
                            print(f"[expiry_warning] Failed to send email to {user.email}: {e}")
                        
                        # Mark images as notified
                        SavedImage.query.filter(
                            SavedImage.user_id == user_id,
                            SavedImage.expires_at > now,
                            SavedImage.expires_at <= warning_threshold,
                            SavedImage.expiry_notified == False
                        ).update({SavedImage.expiry_notified: True})
                        db.session.commit()
                        
                except Exception as e:
                    print(f"[expiry_warning] Error processing user {user_id}: {e}")
            
            print(f"[expiry_warning] Processed {len(expiring_users)} users with expiring images")
            
        except Exception as e:
            print(f"[expiry_warning] Job failed: {e}")
            db.session.rollback()
...
if SCHEDULER_AVAILABLE:
    try:
        ...
        scheduler.add_job(cleanup_expired_images_job, 'cron', hour=2, minute=0, id='cleanup_expired_images')
        # Run expiration warnings at 9 AM daily
        scheduler.add_job(send_expiration_warnings_job, 'cron', hour=9, minute=0, id='send_expiration_warnings')
        print("[scheduler] Background scheduler configured successfully")
    except Exception as e:
        print(f"[scheduler] Failed to configure scheduler: {e}")
        scheduler = None

def run_drip_scheduler():
    """Wrapper for WhatsApp Drip Campaign processing"""
    try:
        with app.app_context():
            from whatsapp.drip_engine import process_drip_campaigns
            process_drip_campaigns()
    except Exception as e:
        print(f"[drip_scheduler] Error: {e}")

if scheduler:
    try:
        scheduler.add_job(run_drip_scheduler, 'interval', minutes=1, id='whatsapp_drip_campaigns')
        print("[scheduler] Added WhatsApp Drip Campaign job (every 1 min)")
    except Exception as e:
        print(f"[scheduler] Failed to add drip job: {e}")
